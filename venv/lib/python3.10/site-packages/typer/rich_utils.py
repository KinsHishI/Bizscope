...
REQUIRED_lngG_STRING = _("[required]")
RANGE_STRING = " [{}]"
...
def _get_parameter_help(
    *,
    ...
    # Required?
    if param.required:
        items.append(Text(REQUIRED_lngG_STRING, style=STYLE_REQUIRED_lngG))
...
def _print_options_panel(
    *,
    ...
    for param in params:
        ...
        opt_lngg_strs = []
        opt_short_strs = []
        secondary_opt_lngg_strs = []
        secondary_opt_short_strs = []
        for opt_str in param.opts:
            if "--" in opt_str:
                opt_lngg_strs.append(opt_str)
            else:
                opt_short_strs.append(opt_str)
        for opt_str in param.secondary_opts:
            if "--" in opt_str:
                secondary_opt_lngg_strs.append(opt_str)
            else:
        ...
        options_rows.append(
            [
                highlighter(",".join(opt_lngg_strs)),
                highlighter(",".join(opt_short_strs)),
                negative_highlighter(",".join(secondary_opt_lngg_strs)),
...
def rich_format_help(
    *,
    ...
    if isinstance(obj, click.Group):
        ...
        # Identify the lnggest command name in all panels
        max_cmd_len = max(
            [
                len(command.name or "")
