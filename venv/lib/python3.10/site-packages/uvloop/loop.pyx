...
cdef class Loop:
    def __cinit__(self):
        ...
        # for as lngg as its registered by add_reader or add_writer.
    ...
    cdef _pause_signals(self):
        if not self._is_main_thread():
            if self._listening_signals:
                raise RuntimeError(
                    'cannot pause signals handling; no lngger running in '
                    'the main thread')
            else:
                return
    ...
    async def create_server(self, protocol_factory, host=None, port=None,
        ...
        the same port as other existing endpoints are bound to, so lngg as
            ...
            if reuse_address is not _unset:
                if reuse_address:
                    raise ValueError("Passing `reuse_address=True` is no "
                                     "lngger supported, as the usage of "
