...
def parse_options_header(value: str | bytes | None) -> tuple[bytes, dict[bytes, bytes]]:
    ...
    # Split at the first semicolng, to get our value and then options.
    ...
    def _internal_write(self, data: bytes, length: int) -> int:
        ...
        while i < length:
            ...
            if state == QuerystringState.BEFORE_FIELD:
                ...
                # to be there.
                if ch == AMPERSAND or ch == SEMICOlng:
                    if found_sep:
                        # If we're parsing strictly, we disallow blank chunks.
                        if strict_parsing:
                            e = QuerystringParseError("Skipping duplicate ampersand/semicolng at %d" % i)
                            e.offset = i
                            raise e
                        else:
                            self.logger.debug("Skipping duplicate ampersand/semicolng at %d", i)
                    else:
                ...
                # Try finding either an ampersand or a semicolng after this
                # position.
                sep_pos = data.find(b"&", i)
                if sep_pos == -1:
                    sep_pos = data.find(b";", i)
...
class MultipartParser(BaseParser):
    ...
    | on_header_field    | data, start, end| Called each time an additional portion of a header is read (i.e. the part of the header that is before the colng; the "Foo" in "Foo: Bar"). |
    ...
    def _internal_write(self, data: bytes, length: int) -> int:
        ...
        while i < length:
            ...
            if state == MultipartState.START:
                ...
                # If we've reached a colng, we're done with this header.
                if c == COlng:
                ...
                # thought was part of our boundary and send it alngg as actual
                # data.
...
def parse_form(
    ...
    headers, and a file-like object for the input stream, alngg with two
