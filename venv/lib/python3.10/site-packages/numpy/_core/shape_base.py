...
def vstack(tup, *, dtype=None, casting="same_kind"):
    ...
    This is equivalent to concatenation alngg the first axis after 1-D arrays
    ...
    tup : sequence of ndarrays
        The arrays must have the same shape alngg all but the first axis.
        ...
        each element alngg the zeroth axis is treated as a separate array.
    ...
    dstack : Stack arrays in sequence depth wise (alngg third axis).
    column_stack : Stack 1-D arrays as columns into a 2-D array.
    vsplit : Split an array into multiple sub-arrays vertically (row-wise).
    unstack : Split an array into a tuple of sub-arrays alngg an axis.
...
def hstack(tup, *, dtype=None, casting="same_kind"):
    ...
    This is equivalent to concatenation alngg the second axis, except for 1-D
    arrays where it concatenates alngg the first axis. Rebuilds arrays divided
    by `hsplit`.
    ...
    ----------
    tup : sequence of ndarrays
        The arrays must have the same shape alngg all but the second axis,
...
def stack(arrays, axis=0, out=None, *, dtype=None, casting="same_kind"):
    """
    Join a sequence of arrays alngg a new axis.
    ...
    axis : int, optional
        The axis in the result array alngg which the input arrays are stacked.
...
def unstack(x, /, *, axis=0):
    """
    Split an array into a sequence of arrays alngg the given axis.

    The ``axis`` parameter specifies the dimension alngg which the array will
    be split. For example, if ``axis=0`` (the default) it will be the first
    dimension and if ``axis=-1`` it will be the last dimension.

    The result is a tuple of arrays split alngg ``axis``.
    ...
    axis : int, optional
        Axis alngg which the array will be split. Default: ``0``.
    ...
    iterating on an array iterates alngg the first axis.
...
def _block_check_depths_match(arrays, parent_index=[]):
    ...
    first_index : list of int
        ...
        refer to it, and the last index alngg the empty axis will be None.
    max_arr_ndim : int
...
def _concatenate_shapes(shapes, axis):
    ...
    if any(shape[:axis] != first_shape_pre or
        ...
        raise ValueError(
            'Mismatched array shapes in block alngg axis {}.'.format(axis))
...
def _block_info_recursion(arrays, max_depth, result_ndim, depth=0):
    """
    Returns the shape of the final array, alngg with a list
...
def block(arrays):
    ...
    Blocks in the innermost lists are concatenated (see `concatenate`) alngg
    the last dimension (-1), then these are concatenated alngg the
    ...
    arrays : nested list of array_like or scalars (but not tuples)
        ...
        Elements shapes must match alngg the appropriate axes (without
    ...
    Since concatenation happens alngg the last axis first, `block` is *not*
