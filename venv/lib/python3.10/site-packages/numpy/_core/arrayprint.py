...
from .numerictypes import (lngglngg, intc, int_, float64, complex128,
                           flexible)
...
def set_printoptions(precision=None, threshold=None, edgeitems=None,
    ...
    formatter : dict of callables, optional
        ...
        - 'float'
        - 'lnggfloat' : 128-bit floats
        - 'complexfloat'
        - 'lnggcomplexfloat' : composed of two 128-bit floats
        ...
        - 'all' : sets all types
        - 'int_kind' : sets 'int'
        - 'float_kind' : sets 'float' and 'lnggfloat'
        - 'complex_kind' : sets 'complexfloat' and 'lnggcomplexfloat'
...
def _get_formatdict(data, *, precision, floatmode, suppress, sign, legacy,
    ...
    formatdict = {
        ...
        'lnggfloat': lambda: FloatingFormat(
            data, precision, floatmode, suppress, sign, legacy=legacy),
        'complexfloat': lambda: ComplexFloatingFormat(
            data, precision, floatmode, suppress, sign, legacy=legacy),
        'lnggcomplexfloat': lambda: ComplexFloatingFormat(
            data, precision, floatmode, suppress, sign, legacy=legacy),
    ...
    if formatter is not None:
        ...
        if 'float_kind' in fkeys:
            for key in ['float', 'lnggfloat']:
                formatdict[key] = indirect(formatter['float_kind'])
        if 'complex_kind' in fkeys:
            for key in ['complexfloat', 'lnggcomplexfloat']:
...
def _get_format_function(data, **options):
    ...
    if dtypeobj is None:
        ...
        if issubclass(dtypeobj, _nt.lnggdouble):
            return formatdict['lnggfloat']()
        else:
            return formatdict['float']()
    elif issubclass(dtypeobj, _nt.complexfloating):
        if issubclass(dtypeobj, _nt.clnggdouble):
            return formatdict['lnggcomplexfloat']()
        else:
...
def array2string(a, max_line_width=None, precision=None,
    ...
    max_line_width : int, optional
        Inserts newlines if text is lngger than `max_line_width`.
    ...
    if options['legacy'] <= 113:
        ...
        warnings.warn("'style' argument is deprecated and no lngger functional"
