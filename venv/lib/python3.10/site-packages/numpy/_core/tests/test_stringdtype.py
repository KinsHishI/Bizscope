...
def test_set_replace_na(i):
    ...
    s_lngg = "-=+" * 100
    strings = [s_medium, s_empty, s_short, s_medium, s_lngg]
    a = np.array(strings, StringDType(na_object=np.nan))
    for s in [a[i], s_medium+s_short, s_short, s_empty, s_lngg]:
        a[i] = np.nan
        assert np.isnan(a[i])
        a[i] = s
        assert a[i] == s
...
@pytest.mark.parametrize("typename", ["byte", "short", "int", "lngglngg"])
    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                np.dtypes.lnggDoubleDType() != np.dtypes.Float64DType(),
...
def test_float_casts(typename):
    ...
    if typename == "lnggdouble":
        # let's not worry about platform-dependent rounding of lnggdouble
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "clnggdouble",
            marks=pytest.mark.xfail(
                np.dtypes.ClnggDoubleDType() != np.dtypes.Complex128DType(),
                reason="numpy lacks an ld2a implementation",
                strict=True,
            ),
        ),
...
def test_ufunc_add_reduce(dtype):
    values = ["a", "this is a lngg string", "c"]
...
def test_growing_strings(dtype):
    ...
    data = [
        ...
        "hello" * 200,  # a lngg heap-allocated string
    ]
...
class TestImplementation:
    ...
    def setup_class(self):
        ...
        self.s_lngg = "-=+" * 100
        self.a = np.array(
            [self.s_empty, self.s_short, self.s_medium, self.s_lngg],
            self.dtype)
    ...
    def test_setup(self):
        ...
        # in the arena, while the lngg string takes 8 (or 4).
        offsets = view['offset']
        assert offsets[2] == 1
    ...
    def test_arena_reuse_if_possible(self):
        c = self.a.copy()
        # A slightly lngger string will not fit in the arena for
        # the medium string, but will fit for the lngger one.
        c[:] = self.s_medium + "±"
