...
class TestComplexArray:
    def test_str(self):
        ...
        dtypes = [np.complex64, np.cdouble, np.clnggdouble]
...
class TestArray2String:
    ...
    def test_multiline_repr(self):
        ...
        a = np.array([[None, MultiLinelngg()], [MultiLinelngg(), None]])
        assert_equal(
            repr(a),
            'array([[None, Line 1\n'
            '              LooooooooooongestLine2\n'
            ...
            '       [Line 1\n'
            '        LooooooooooongestLine2\n'
            '        lnggerLine 3          , None]], dtype=object)'
        )
        assert_equal(
            np.array_repr(a, 20),
            'array([[None,\n'
...
def test_scalar_repr_numbers(dtype, value):
    ...
    if dtype.char == "g":
        assert representation == f"np.lnggdouble('{repr_string}')"
    elif dtype.char == 'G':
        assert representation == f"np.clnggdouble('{repr_string}')"
    else:
