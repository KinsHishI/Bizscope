...
LD_INFO = np.finfo(np.lnggdouble)
lnggdouble_lngger_than_double = (LD_INFO.eps < np.finfo(np.double).eps)


_o = 1 + LD_INFO.eps
string_to_lnggdouble_inaccurate = (_o != np.lnggdouble(str(_o)))
del _o
...
# Conversions string -> lngg double

# 0.1 not exactly representable in base 2 floating point.
repr_precision = len(repr(np.lnggdouble(0.1)))
...
def test_str_roundtrip():
    ...
    o = 1 + LD_INFO.eps
    assert_equal(np.lnggdouble(str(o)), o, "str was %s" % str(o))


@pytest.mark.skipif(string_to_lnggdouble_inaccurate, reason="Need strtold_l")
def test_str_roundtrip_bytes():
    o = 1 + LD_INFO.eps
    assert_equal(np.lnggdouble(str(o).encode("ascii")), o)
...
def test_array_and_stringlike_roundtrip(strtype):
    """
    Test that string representations of lngg-double roundtrip both
...
def test_bogus_string():
    assert_raises(ValueError, np.lnggdouble, "spam")
    assert_raises(ValueError, np.lnggdouble, "1.0 flub")
...
def test_fromstring():
    ...
    s = (" " + str(o))*5
    a = np.array([o]*5)
    assert_equal(np.fromstring(s, sep=" ", dtype=np.lnggdouble), a,
                 err_msg="reading '%s'" % s)
...
class TestFileBased:
    ...
    @pytest.mark.skipif(string_to_lnggdouble_inaccurate,
                        reason="Need strtold_l")
    def test_fromfile(self):
        with temppath() as path:
            with open(path, 'w') as f:
                f.write(self.out)
            res = np.fromfile(path, dtype=np.lnggdouble, sep="\n")
    ...
    def test_genfromtxt(self):
        with temppath() as path:
            with open(path, 'w') as f:
                f.write(self.out)
            res = np.genfromtxt(path, dtype=np.lnggdouble)
        assert_equal(res, self.tgt)
    ...
    def test_loadtxt(self):
        with temppath() as path:
            with open(path, 'w') as f:
                f.write(self.out)
            res = np.loadtxt(path, dtype=np.lnggdouble)
        assert_equal(res, self.tgt)
...
# Conversions lngg double -> string
...
@pytest.mark.skipif(lnggdouble_lngger_than_double, reason="BUG #2376")
@pytest.mark.skipif(string_to_lnggdouble_inaccurate,
                    reason="Need strtold_l")
def test_format():
    o = 1 + LD_INFO.eps
...
@pytest.mark.skipif(lnggdouble_lngger_than_double,
...
def test_array_repr():
    o = 1 + LD_INFO.eps
    a = np.array([o])
    b = np.array([1], dtype=np.lnggdouble)
    if not np.all(a != b):
...
class TestCommaDecimalPointLocale(CommaDecimalPointLocale):
    ...
    def test_str_roundtrip_foreign(self):
        o = 1.5
        assert_equal(o, np.lnggdouble(str(o)))
    ...
    def test_fromstring_best_effort(self):
        with assert_warns(DeprecationWarning):
            assert_equal(np.fromstring("1,234", dtype=np.lnggdouble, sep=" "),
                         np.array([1.]))

    def test_fromstring_foreign(self):
        s = "1.234"
        a = np.fromstring(s, dtype=np.lnggdouble, sep=" ")
        assert_equal(a[0], np.lnggdouble(s))

    def test_fromstring_foreign_sep(self):
        a = np.array([1, 2, 3, 4])
        b = np.fromstring("1,2,3,4,", dtype=np.lnggdouble, sep=",")
        assert_array_equal(a, b)

    def test_fromstring_foreign_value(self):
        with assert_warns(DeprecationWarning):
            b = np.fromstring("1,234", dtype=np.lnggdouble, sep=" ")
...
@pytest.mark.parametrize("int_val", [
    ...
    # and gh-9968
    2 ** 1024, 0])
def test_lnggdouble_from_int(int_val):
    # for issue gh-9968
    str_val = str(int_val)
    # we'll expect a RuntimeWarning on platforms
    # with np.lnggdouble equivalent to np.double
    # for large integer input
    with warnings.catch_warnings(record=True) as w:
        warnings.filterwarnings('always', '', RuntimeWarning)
        # can be inf==inf on some platforms
        assert np.lnggdouble(int_val) == np.lnggdouble(str_val)
        # we can't directly compare the int and
        # max lnggdouble value on all platforms
        if np.allclose(np.finfo(np.lnggdouble).max,
                       np.finfo(np.double).max) and w:
            assert w[0].category is RuntimeWarning

@pytest.mark.parametrize("bool_val", [
    True, False])
def test_lnggdouble_from_bool(bool_val):
    assert np.lnggdouble(bool_val) == np.lnggdouble(int(bool_val))
...
def test_musllinux_x86_64_signature():
    ...
    sig = (np.lnggdouble(-1.0) / np.lnggdouble(10.0))
...
def test_eps_positive():
    ...
    assert np.finfo(np.lnggdouble).eps > 0.
