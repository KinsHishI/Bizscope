...
class TestFloatExceptions:
    ...
    def test_floating_exceptions(self, typecode):
        if 'bsd' in sys.platform and typecode in 'gG':
            pytest.skip(reason="Fallback impl for (c)lnggdouble may not raise "
...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        f32, f64, fld = np.float32(0), np.float64(0), np.lnggdouble(0)
        c64, c128, cld = np.complex64(0), np.complex128(0), np.clnggdouble(0)
        ...
        assert_equal(promote_func(fld, f32), np.dtype(np.lnggdouble))
        assert_equal(promote_func(f64, fld), np.dtype(np.lnggdouble))
        assert_equal(promote_func(c128, c64), np.dtype(np.complex128))
        assert_equal(promote_func(cld, c128), np.dtype(np.clnggdouble))
        assert_equal(promote_func(c64, fld), np.dtype(np.clnggdouble))
        ...
        assert_equal(promote_func(cld, f64), np.dtype(np.clnggdouble))
    ...
    def test_coercion(self):
        ...
        for a in [np.array([True, False]), np.array([-3, 12], dtype=np.int8)]:
            ...
            b = np.lnggdouble(1.234) * a
            assert_equal(b.dtype, np.dtype(np.lnggdouble),
            ...
            b = np.clnggdouble(1.234j) * a
            assert_equal(b.dtype, np.dtype(np.clnggdouble),
                         "array type %s" % a.dtype)
    ...
    def test_can_cast_and_promote_usertypes(self):
        ...
        # int64 can be both lngg and lngglngg and it registers only the first)
...
class TestFromiter:
    ...
    def test_empty_not_structured(self, dtype):
        # Note, "S0" could be allowed at some point, so lngg "S" (without
...
class TestIsclose:
    ...
    def test_timedelta(self):
        # Allclose currently works for timedelta64 as lngg as `atol` is
