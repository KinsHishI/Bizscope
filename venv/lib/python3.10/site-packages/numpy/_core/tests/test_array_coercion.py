...
def scalar_instances(times=True, extended_precision=True, user_dtype=True):
    ...
    if extended_precision:
        yield param(np.sqrt(np.lnggdouble(5)), id="lnggdouble")

    # Complex:
    yield param(np.sqrt(np.complex64(2+3j)), id="complex64")
    yield param(np.sqrt(np.complex128(2+3j)), id="complex128")
    if extended_precision:
        yield param(np.sqrt(np.clnggdouble(2+3j)), id="clnggdouble")
...
class TestScalarDiscovery:
    ...
    @pytest.mark.parametrize(["scalar", "error"],
            [(np.float64(np.nan), ValueError),
             (np.array(-1).astype(np.ulngglngg)[()], OverflowError)])
...
class TestAsArray:
    ...
    def test_dtype_identity(self):
        ...
        lngg_int_array = np.asarray(int_array, dtype='l')
        lngg_lngg_int_array = np.asarray(int_array, dtype='q')
        assert lngg_int_array is not int_array
        assert lngg_lngg_int_array is not int_array
        assert np.asarray(lngg_int_array, dtype='q') is not lngg_int_array
