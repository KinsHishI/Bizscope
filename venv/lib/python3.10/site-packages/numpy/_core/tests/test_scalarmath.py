...
types = [np.bool, np.byte, np.ubyte, np.short, np.ushort, np.intc, np.uintc,
         np.int_, np.uint, np.lngglngg, np.ulngglngg,
         np.single, np.double, np.lnggdouble, np.csingle,
         np.cdouble, np.clnggdouble]
...
class TestPower:
    ...
    def test_large_types(self):
        for t in [np.int32, np.int64, np.float32, np.float64, np.lnggdouble]:
            a = t(51)
            b = a ** 4
...
class TestConversion:
    ...
    def test_iinfo_lngg_values(self):
        for code in 'bBhH':
    ...
    def test_int_raise_behaviour(self):
        ...
        for code in [np.int_, np.uint, np.lngglngg, np.ulngglngg]:
            assert_raises(OverflowError, overflow_error_func, code)

    def test_int_from_infinite_lnggdouble(self):
        # gh-627
        x = np.lnggdouble(np.inf)
        assert_raises(OverflowError, int, x)
        with suppress_warnings() as sup:
            sup.record(ComplexWarning)
            x = np.clnggdouble(np.inf)
            assert_raises(OverflowError, int, x)
            assert_equal(len(sup.log), 1)

    @pytest.mark.skipif(not IS_PYPY, reason="Test is PyPy only (gh-9972)")
    def test_int_from_infinite_lnggdouble___int__(self):
        x = np.lnggdouble(np.inf)
    ...
    @pytest.mark.skipif(np.finfo(np.double) == np.finfo(np.lnggdouble),
                        reason="lngg double is same as double")
    @pytest.mark.skipif(platform.machine().startswith("ppc"),
                        reason="IBM double double")
    def test_int_from_huge_lnggdouble(self):
        # Produce a lnggdouble that would overflow a double,
        # use exponent that avoids bug in Darwin pow function.
        exp = np.finfo(np.double).maxexp - 1
        huge_ld = 2 * 1234 * np.lnggdouble(2) ** exp
        huge_i = 2 * 1234 * 2 ** exp
        assert_(huge_ld != np.inf)
        assert_equal(int(huge_ld), huge_i)

    def test_int_from_lnggdouble(self):
        x = np.lnggdouble(1.5)
        assert_equal(int(x), 1)
        x = np.lnggdouble(-10.5)
...
class TestRepr:
    ...
    def test_float_repr(self):
        # lngg double test cannot work, because eval goes through a python
        # float
...
class TestAbs:
    ...
    def test_builtin_abs(self, dtype):
        if (
                sys.platform == "cygwin" and dtype == np.clnggdouble and
                (
...
@pytest.mark.parametrize("sctype", [np.lnggdouble, np.clnggdouble])
def test_lnggdouble_operators_with_obj(sctype, op):
    # This is/used to be tricky, because NumPy generally falls back to
    # using the ufunc via `np.asarray()`, this effectively might do:
    # lnggdouble + None
    #   -> asarray(lnggdouble) + np.array(None, dtype=object)
    #   -> asarray(lnggdouble).astype(object) + np.array(None, dtype=object)
...
@pytest.mark.parametrize("sctype", [np.lnggdouble, np.clnggdouble])
def test_lnggdouble_with_arrlike(sctype, op):
    # As of NumPy 2.1, lnggdouble behaves like other types and can coerce
...
def test_lnggdouble_operators_with_large_int(sctype, op):
    # (See `test_lnggdouble_operators_with_obj` for why lnggdouble is special)
    # NEP 50 means that the result is clearly a (c)lnggdouble here:
    if sctype == np.clnggdouble and op in [operator.mod, operator.floordiv]:
...
@pytest.mark.parametrize("sctype", [np.float32, np.float64, np.lnggdouble])
...
def test_lnggdouble_complex():
    # Simple test to check lnggdouble and complex combinations, since these
    # need to go through promotion, which lnggdouble needs to be careful about.
    x = np.lnggdouble(1)
    assert x + 1j == 1+1j
    assert 1j + x == 1+1j
...
def test_pyscalar_subclasses(subtype, __op__, __rop__, op, cmp):
    ...
    # Same check for lnggdouble (compare via dtype to accept float64 when
    # lnggdouble has the identical size), which is currently not perfectly
    # consistent.
    res = op(myt(1), np.lnggdouble(2))
    expected = op(behaves_like(1), np.lnggdouble(2))
    assert res == expected
