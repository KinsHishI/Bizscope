...
def interesting_binop_operands(val1, val2, dtype):
    ...
    * lngger array (SIMD) placing the value of interest at different positions
...
class TestLog:
    ...
    def test_log_values_maxofdtype(self):
        ...
        # other non x86-64 platforms checking `lnggdouble` isn't too useful as
        # it's an alias for float64.
        if platform.machine() == 'x86_64':
            dtypes += [np.lnggdouble]
    ...
    def test_avx_based_ufunc(self):
        ...
        for func, prop in avx_ufuncs.items():
            ...
            for size in range(1,32):
                ...
                x_f128 = x_f32.astype(np.lnggdouble)
...
class TestLdexp:
    def _check_ldexp(self, tp):
        ...
        assert_almost_equal(ncu.ldexp(np.array(2., np.float64),
                                      np.array(3, tp)), 16.)
        assert_almost_equal(ncu.ldexp(np.array(2., np.lnggdouble),
                                      np.array(3, tp)), 16.)
...
class TestMaximum(_FilterInvalids):
    ...
    def test_precision(self):
        dtypes = [np.float16, np.float32, np.float64, np.lnggdouble]
...
class TestComplexFunctions:
    ...
    @pytest.mark.parametrize('dtype', [
        np.complex64, np.complex128, np.clnggdouble
    ])
    def test_loss_of_precision(self, dtype):
        """Check loss of precision in complex arc* functions"""
        if dtype is np.clnggdouble and platform.machine() != 'x86_64':
        ...
        if dtype is np.clnggdouble:
            if bad_arcsinh():
                pytest.skip("Trig functions of np.clnggdouble values known "
                            "to be inaccurate on aarch64 and PPC for some "
                            "compilation configurations.")
            # It's not guaranteed that the system-provided arc functions
            # are accurate down to a few epsilngs. (Eg. on Linux 64-bit)
            # So, give more leeway for lngg complex tests here:
...
class TestAttributes:
    ...
    def test_doc(self):
        # don't bother checking the lngg list of kwargs, which are likely to
        # change
...
def test_nextafterl():
    return _test_nextafter(np.lnggdouble)
...
@pytest.mark.xfail(condition=platform.machine().startswith("ppc64"),
                    reason="IBM double double")
def test_spacingl():
    return _test_spacing(np.lnggdouble)
...
def test_nextafter_vs_spacing():
    # XXX: spacing does not handle lngg double yet
