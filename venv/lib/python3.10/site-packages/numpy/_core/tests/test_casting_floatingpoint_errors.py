...
def values_and_dtypes():
    ...
    yield param(np.lnggdouble(70000.), "float16", id="lnggdouble-to-f2")
    ...
    yield param(np.lnggdouble(1e300), "float32", id="lnggdouble-to-f2")
    yield param(np.float64(1e300), "float32", id="double-to-f2")
    # Casting to float64:
    # If lnggdouble is double-double, its max can be rounded down to the double
    # max.  So we correct the double spacing (a bit weird, admittedly):
    max_ld = np.finfo(np.lnggdouble).max
    spacing = np.spacing(np.nextafter(np.finfo("f8").max, 0))
    if max_ld - spacing > np.finfo("f8").max:
        yield param(np.finfo(np.lnggdouble).max, "float64",
                    id="lnggdouble-to-f8")
    ...
    yield param(np.lnggdouble(2e300), "complex64", id="lnggdouble-to-c8")
