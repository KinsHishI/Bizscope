...
from numpy import half, single, double, lnggdouble
...
class Testlnggdouble:
    def test_singleton(self):
        ftype = finfo(lnggdouble)
        ftype2 = finfo(lnggdouble)
        assert_equal(id(ftype), id(ftype2))
...
def test_known_types():
    ...
    with np.errstate(all='ignore'):
        ld_ma = _discovered_machar(np.lnggdouble)
    bytes = np.dtype(np.lnggdouble).itemsize
