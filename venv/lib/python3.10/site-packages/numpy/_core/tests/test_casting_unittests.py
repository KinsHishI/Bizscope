...
# Simple skips object, parametric and lngg double (unsupported by struct)
...
def get_expected_stringlength(dtype):
    ...
    # Note: Can't do dtype comparison for lnggdouble on windows
    if dtype.char == "g":
        return 48
    elif dtype.char == "G":
        return 48 * 2
...
class TestChanges:
    ...
    def test_float_to_string(self, floating, string):
        ...
        # 100 is lngg enough to hold any formatted floating
...
class TestCasting:
    ...
    def test_simple_direct_casts(self, from_dt):
        ...
        lnggdouble and Clnggdouble are tested, but only using double precision.
    ...
    def test_string_cancast(self, other_DT, string_char):
        ...
        # check the cast-safety for too lngg/fixed string lengths:
    ...
    def test_simple_string_casts_roundtrip(self, other_dt, string_char):
        ...
        string_dt_lngg = self.string_with_modified_length(string_dt, 1)
        str_arr_lngg = np.zeros(len(orig_arr), dtype=string_dt_lngg)
        ...
        for contig in [True, False]:
            ...
            _, str_arr_lngg = self.get_data_variation(
                orig_arr, str_arr_lngg, True, contig)
            ...
            cast._simple_strided_call((other_arr, str_arr_lngg))
            assert_array_equal(str_arr, str_arr_lngg)
            ...
            other_arr[...] = 0
            cast_back._simple_strided_call((str_arr_lngg, other_arr))
            assert_array_equal(orig_arr, other_arr)
