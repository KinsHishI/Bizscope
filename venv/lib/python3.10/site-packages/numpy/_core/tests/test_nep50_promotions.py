...
def test_nep50_weak_integers_with_inexact(dtype):
    ...
    if dtype in "dDG":
        ...
        # NumPy uses (or used) `int -> string -> lnggdouble` for the
        ...
        if dtype in "gG":
            try:
                ...
                pytest.skip("`huge_int -> string -> lnggdouble` failed")
        ...
        with pytest.warns(RuntimeWarning):
            ...
            # double instead of the lnggdouble loop.  That leads to slightly
...
def test_nep50_integer_conversion_errors():
    ...
    # Error message depends on platform (maybe unsigned int or unsigned lngg)
    with pytest.raises(OverflowError,
