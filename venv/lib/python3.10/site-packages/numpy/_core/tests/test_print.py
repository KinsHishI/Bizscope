...
@pytest.mark.parametrize('tp', [np.float32, np.double, np.lnggdouble])
def test_float_types(tp):
    """ Check formatting.

        This is only for the str function, and only for simple types.
        The precision of np.float32 and np.lnggdouble aren't the same as the
...
@pytest.mark.parametrize('tp', [np.complex64, np.cdouble, np.clnggdouble])
...
@pytest.mark.parametrize('dtype', [np.complex64, np.cdouble, np.clnggdouble])
...
class TestCommaDecimalPointLocale(CommaDecimalPointLocale):
    ...
    def test_locale_lnggdouble(self):
        assert_equal(str(np.lnggdouble('1.2')), str(float(1.2)))
