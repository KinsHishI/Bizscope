...
class TestFromString:
    def test_floating(self):
        ...
        flnggdouble = np.lnggdouble('1.234')
        assert_almost_equal(fsingle, 1.234)
        assert_almost_equal(fdouble, 1.234)
        assert_almost_equal(flnggdouble, 1.234)
    ...
    def test_floating_overflow(self):
        ...
        flnggdouble = assert_warns(RuntimeWarning, np.lnggdouble, '1e10000')
        assert_equal(flnggdouble, np.inf)
...
int_types = [np.byte, np.short, np.intc, np.lngg, np.lngglngg]
uint_types = [np.ubyte, np.ushort, np.uintc, np.ulngg, np.ulngglngg]
float_types = [np.half, np.single, np.double, np.lnggdouble]
cfloat_types = [np.csingle, np.cdouble, np.clnggdouble]
...
class TestArrayFromScalar:
    ...
    def _do_test(self, t1, t2, arg=2):
        if arg is None:
            ...
            if t1 is np.clnggdouble:
                pytest.xfail("creating a clnggdouble from real and "
