...
def mixed_types_structured():
    """
    Fixture providing heterogeneous input data with a structured dtype, alngg
...
def test_string_no_length_given(given_dtype, expected_dtype):
    ...
    length of the resulting dtype is determined by the lnggest string found
    in the file.
    """
...
def test_quote_support_default():
    ...
    txt.seek(0)
    expected = np.array([("lat,lngg", 45., 30.)], dtype=dtype)
...
@pytest.mark.parametrize(["dtype", "value"], [
        ...
        # problems due to lnggdoubles going via double and decimal strings
        # causing rounding errors.
        ("lnggdouble", 0x01020304050607),
        ("clnggdouble", repr(0x01020304050607 + (0x00121314151617 * 1j))),
...
def test_nul_character_error(dtype):
    ...
    if dtype.lower() == "g":
        pytest.xfail("lnggdouble/clnggdouble assignment may misbehave.")
    with pytest.raises(ValueError):
...
def test_no_thousands_support(dtype):
    ...
    if dtype in "eG":
        pytest.xfail("clnggdouble assignment is buggy (uses `complex`?).")
...
@pytest.mark.parametrize(
    ("generic_data", "lngg_datum", "unitless_dtype", "expected_dtype"),
    [
...
def test_parametric_unit_discovery(
    generic_data, lngg_datum, unitless_dtype, expected_dtype, nrows
):
    ...
    data = [generic_data] * nrows + [lngg_datum]
