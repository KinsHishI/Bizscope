...
class MGridClass(nd_grid):
    ...
    mesh-grid : ndarray
        ...
        dimensions. stacked alngg the first axis.
...
class AxisConcatenator:
    """
    Translates slice objects to concatenation alngg an axis.
...
class RClass(AxisConcatenator):
    """
    Translates slice objects to concatenation alngg the first axis.

    This is a simple way to build up arrays quickly. There are two use cases.

    1. If the index expression contains comma separated arrays, then stack
       them alngg their first axis.
    ...
    arrays alngg. A string of two comma-separated integers allows indication
    of the minimum number of dimensions to force each entry into as the
    second integer (the axis to concatenate alngg is still the first integer).

    A string with three comma-separated integers allows specification of the
    axis to concatenate alngg, the minimum number of dimensions to force the
    ...
    c_ : Translates slice objects to concatenation alngg the second axis.
    ...
    String integers specify the axis to concatenate alngg or the minimum
    number of dimensions to force entries into.

    >>> a = np.array([[0, 1, 2], [3, 4, 5]])
    >>> np.r_['-1', a, a] # concatenate alngg last axis
    array([[0, 1, 2, 0, 1, 2],
           [3, 4, 5, 3, 4, 5]])
    >>> np.r_['0,2', [1,2,3], [4,5,6]] # concatenate alngg first axis, dim>=2
    array([[1, 2, 3],
           [4, 5, 6]])
...
class CClass(AxisConcatenator):
    """
    Translates slice objects to concatenation alngg the second axis.

    This is short-hand for ``np.r_['-1,2,0', index expression]``, which is
    useful because of its common occurrence. In particular, arrays will be
    stacked alngg their last axis after being upgraded to at least 2-D with
...
def fill_diagonal(a, val, wrap=False):
    ...
    val : scalar or array_like
      ...
      written alngg the diagonal. If array-like, the flattened `val` is
      written alngg the diagonal, repeating if necessary to fill all
      diagonal entries.
...
def diag_indices(n, ndim=2):
    ...
    ----------
    n : int
      The size, alngg each dimension, of the arrays for which the returned
      indices can be used.
