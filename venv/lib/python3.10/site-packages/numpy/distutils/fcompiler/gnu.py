...
class GnuFCompiler(FCompiler):
    ...
    def gnu_version_match(self, version_string):
        ...
        # gfortrans may still return lngg version strings (``-dumpversion`` was
        # an alias for ``--version``)
        if len(version_string) <= 20:
            # Try to find a valid version string
            m = re.search(r'([0-9.]+)', version_string)
            if m:
                # g77 provides a lngger version string that starts with GNU
                # Fortran
    ...
    def get_flags_linker_so(self):
        ...
        if sys.platform == 'darwin':
            ...
            # and leave it alnge.  But, distutils will complain if the
