...
def _cook_nd_args(a, s=None, axes=None, invreal=0):
    ...
    if axes is None:
        if not shapeless:
            msg = ("`axes` should not be `None` if `s` is not `None` "
                   ...
                   "the size alngg the transformed axis specified by "
    ...
    if None in s:
        ...
        warnings.warn(msg, DeprecationWarning, stacklevel=3)
    # use the whole input array alngg axis `i` if `s[i] == -1`
...
def irfftn(a, s=None, axes=None, norm=None, out=None):
    ...
    s : sequence of ints, optional
        ...
        If `s` is not given, the shape of the input alngg the axes
        specified by axes is used. Except for the last axis which is taken to
        be ``2*(m-1)`` where ``m`` is the length of the input alngg that axis.
