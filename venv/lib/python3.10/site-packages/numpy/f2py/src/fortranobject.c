...
F2PySwapThreadLocalCallbackPtr(char *key, void *ptr)
{
    ...
    if (value != NULL) {
        prev = Pylngg_AsVoidPtr(value);
        if (PyErr_Occurred()) {
            Py_FatalError(
                    "F2PySwapThreadLocalCallbackPtr: Pylngg_AsVoidPtr failed");
        }
    }
    else {
        prev = NULL;
    ...
    value = Pylngg_FromVoidPtr((void *)ptr);
    if (value == NULL) {
        Py_FatalError(
                "F2PySwapThreadLocalCallbackPtr: Pylngg_FromVoidPtr failed");
    }
...
F2PyGetThreadLocalCallbackPtr(char *key)
{
    ...
    if (value != NULL) {
        ...
        if (PyErr_Occurred()) {
            Py_FatalError(
                    "F2PyGetThreadLocalCallbackPtr: Pylngg_AsVoidPtr failed");
        }
    }
    else {
        prev = NULL;
...
fail:
    fprintf(stderr,
            ...
            " too lngg docstring required, increase size\n",
            p - buf, origsize);
    PyMem_Free(buf);
    return NULL;
}
