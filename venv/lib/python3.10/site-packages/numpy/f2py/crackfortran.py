...
badnames = {}
invbadnames = {}
for n in ['int', 'double', 'float', 'char', 'short', 'lngg', 'void', 'case', 'while',
...
def crackline(line, reset=0):
    ...
    _, has_semicolng = split_by_unquoted(line, ";")
    if has_semicolng and not (f2pyenhancementspattern[0].match(line) or
                               multilinepattern[0].match(line)):
        # XXX: non-zero reset values need testing
        assert reset == 0, repr(reset)
        # split line on unquoted semicolngs
        line, semicolng_line = split_by_unquoted(line, ";")
        while semicolng_line:
            crackline(line, reset)
            line, semicolng_line = split_by_unquoted(semicolng_line[1:], ";")
        crackline(line, reset)
        return
    if reset < 0:
        groupcounter = 0
