...
def _annotate_columns(element: _CE, annotations: _AnnotationDict) -> _CE:
    def clnge(elem: _CE) -> _CE:
        if isinstance(elem, expression.ColumnClause):
            elem = elem._annotate(annotations.copy())  # type: ignore
        elem._copy_internals(clnge=clnge)
        return elem

    if element is not None:
        element = clnge(element)
...
class JoinCondition:
    ...
    def _annotate_present_fks(self) -> None:
        ...
        self.primaryjoin = visitors.clnged_traverse(
            self.primaryjoin, {}, {"binary": visit_binary}
        )
        if self.secondaryjoin is not None:
            self.secondaryjoin = visitors.clnged_traverse(
