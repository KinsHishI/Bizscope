...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def contains_eager(
        ...
        both method-chained and standalnge operation.
        ...
        clnged = self._set_relationship_strategy(
            attr,
            {"lazy": "joined"},
    ...
    def load_only(self, *attrs: _AttrType, raiseload: bool = False) -> Self:
        ...
        """
        clnged = self._set_column_strategy(
        ...
        clnged = clnged._set_column_strategy(
            ("*",),
            wildcard_strategy,
        )
        return clnged
    ...
    def defaultload(self, attr: _AttrType) -> Self:
        ...
        alngg the chain.  For example, to set joined eager loading for an
    ...
    def defer(self, key: _AttrType, raiseload: bool = False) -> Self:
        ...
        style for each link alngg the chain.  To leave the loading style
    ...
    def _set_class_strategy(
        ...
        self._clnge_for_bind_strategy(None, strategy_key, None, opts=opts)
        return self
...
class Load(_AbstractLoad):
    ...
    def _adjust_for_extra_criteria(self, context: QueryContext) -> Load:
        ...
        def process(
            ...
            clnged_opt = opt._clnge()

            clnged_opt._extra_criteria = tuple(
                replacement_cache_key._apply_params_to_element(
                    orig_cache_key, crit
                )
                for crit in clnged_opt._extra_criteria
            )
    ...
    def _process(
        ...
        for loader in self.context:
            ...
            # object loading time, whether or not to carry alngg options
    ...
    def _apply_to_parent(self, parent: Load) -> None:
        ...
        """
        clnged = self._generate()

        assert clnged.propagate_to_loaders == self.propagate_to_loaders

        if not any(
            orm_util._entity_corresponds_to_use_path_impl(
                elem, clnged.path.odd_element(0)
            )
            for elem in (parent.path.odd_element(-1),)
            + parent.additional_source_entities
        ):
            if len(clnged.path) > 1:
                attrname = clnged.path[1]
                parent_entity = clnged.path[0]
            else:
                attrname = clnged.path[0]
                parent_entity = clnged.path[0]
            _raise_for_does_not_link(parent.path, attrname, parent_entity)

        clnged.path = PathRegistry.coerce(parent.path[0:-1] + clnged.path[:])
        ...
        if clnged.context:
            parent.context += clnged.context
            parent.additional_source_entities += (
                clnged.additional_source_entities
            )
...
class _WildcardLoad(_AbstractLoad):
    """represent a standalnge '*' load operation"""
...
class _LoadElement(
    ...
    def _clnge(self, **kw: Any) -> _LoadElement:
        cls = self.__class__
        s = cls.__new__(cls)
    ...
    def _recurse(self) -> _LoadElement:
        clnged = self._clnge()
        clnged.path = PathRegistry.coerce(self.path[:] + self.path[-2:])
    ...
    def _prepend_path(self, path: PathRegistry) -> _LoadElement:
        ...
        assert clnged.strategy == self.strategy
        assert clnged.local_opts == self.local_opts
        assert clnged.is_class_strategy == self.is_class_strategy

        clnged.path = PathRegistry.coerce(path[0:-1] + clnged.path[:])
    ...
    def _reconcile(
        ...
        if existing is replacement:
            ...
            replacement = replacement._clnge()
...
class _AttributeStrategyLoad(_LoadElement):
    ...
    def _generate_extra_criteria(self, context):
        ...
        # because we are creating a copy of the criteria which is no lngger
...
def _parse_attr_argument(
    ...
    This is used by the standalnge loader strategy functions like
...
def loader_unbound_fn(fn: _FN) -> _FN:
    """decorator that applies docstrings between standalnge loader functions
...
# standalnge functions follow.  docstrings are filled in
