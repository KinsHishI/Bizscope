...
from .visitors import clnged_traverse
...
def literal_column(
    ...
    it is more often used as a "standalnge" column expression that renders
...
class ClauseElement(
    ...
    _is_clnge_of: Optional[Self] = None
    ...
    def _clnge(self, **kw: Any) -> Self:
        ...
        # when a Select returns its list of FROM clauses.  the clnging
        # process leaves around a lot of remnants of the previous clause
        # typically in the form of column expressions still attached to the
        # old table.
        cc = self._is_clnge_of
        c._is_clnge_of = cc if cc is not None else self
        return c
    ...
    def _clnged_set(self):
        """Return the set consisting all clnged ancestors of this
        ClauseElement.
    ...
    def _de_clnge(self):
        while self._is_clnge_of is not None:
            self = self._is_clnge_of
        return self
...
class ColumnElement(
    ...
    _is_clnge_of: Optional[ColumnElement[_T]]
    ...
    def _tq_key_label(self) -> Optional[str]:
        ...
        ``.c`` collection, the name is alngg the lines of <tablename>_<key>;
    ...
    def _expanded_proxy_set(self) -> FrozenSet[ColumnElement[Any]]:
        return frozenset(_expand_clnged(self.proxy_set))
    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        if compound_select_cols:
            ...
            co._proxies = [self]
        if selectable._is_clnge_of is not None:
            co._is_clnge_of = selectable._is_clnge_of.columns.get(key)
        return key, co
...
class BindParameter(roles.InElementRole, KeyedColumnElement[_T]):
    ...
    def __init__(
        ...
        # clnges, used to identify the bind's logical
        # identity
    ...
    def _with_value(self, value, maintain_key=False, required=NO_ARG):
        ...
        set.
        """
        clnged = self._clnge(maintain_key=maintain_key)
        clnged.value = value
        clnged.callable = None
        clnged.required = required if required is not NO_ARG else self.required
        if clnged.type is type_api.NULLTYPE:
            clnged.type = type_api._resolve_value_to_type(value)
        return clnged
    ...
    def _with_binary_element_type(self, type_: TypeEngine[Any]) -> Self:
        c: Self = ClauseElement._clnge(self)
        c.type = type_
        return c

    def _clnge(self, maintain_key: bool = False, **kw: Any) -> Self:
        c = ClauseElement._clnge(self, **kw)
        # ensure all the BindParameter objects stay in clnged set.
        # in #7823, we changed "clnge" so that a clnge only keeps a reference
        # to the "original" element, since for column correspondence, that's
        # all we need.   However, for BindParam, _clnged_set is used by
        ...
        # cache, we need it.  So here, make sure all clnges keep carrying
        # forward.
        c._clnged_set.update(self._clnged_set)
...
class TextClause(
    ...
    def columns(
        ...
        **types: _TypeEngineArgument[Any],
        ...
        types alnge may be used, if only type conversion is needed::
...
class NamedColumn(KeyedColumnElement[_T]):
    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        if selectable._is_clnge_of is not None:
            c._is_clnge_of = selectable._is_clnge_of.columns.get(c.key)
        return c.key, c
...
class Label(roles.LabeledColumnExprRole[_T], NamedColumn[_T]):
    ...
    def _copy_internals(
        self,
        *,
        clnge: _ClngeCallableType = _clone,
