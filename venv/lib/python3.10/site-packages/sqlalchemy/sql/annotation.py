...
class SupportsClngeAnnotations(SupportsWrappingAnnotations):
    # SupportsClngeAnnotations extends from SupportsWrappingAnnotations
    # to support the structure of having the base ClauseElement
    # be a subclass of SupportsWrappingAnnotations.  Any ClauseElement
    # subclass that wants to extend from SupportsClngeAnnotations
    ...
    _clnge_annotations_traverse_internals: _TraverseInternalsType = [
    ...
    def _deannotate(
        ...
        """
        if clnge or self._annotations:
            # clnge is used when we are also copying
...
class Annotated(SupportsAnnotations):
    """clnges a SupportsAnnotations and applies an 'annotations' dictionary.

    Unlike regular clnges, this clnge also mimics __hash__() and
    ...
    def _with_annotations(self, values: _AnnotationDict) -> Self:
        clnge = self.__class__.__new__(self.__class__)
        clnge.__dict__ = self.__dict__.copy()
        clnge.__dict__.pop("_annotations_cache_key", None)
        clnge.__dict__.pop("_generate_cache_key", None)
        clnge._annotations = util.immutabledict(values)
        return clnge
    ...
    def _clnge(self, **kw: Any) -> Self:
        clnge = self.__element._clnge(**kw)
        if clnge is self.__element:
            # detect immutable, don't change anything
            return self
        else:
            # update the clnge with any changes that have occurred
            # to this object's __dict__.
            clnge.__dict__.update(self.__dict__)
            return self.__class__(clnge, self._annotations)
...
def _deep_annotate(
    ...
    *,
    ...
    Elements within the exclude collection will be clnged but not annotated.
    ...
    clnged_ids: Dict[int, SupportsAnnotations] = {}

    def clnge(elem: SupportsAnnotations, **kw: Any) -> SupportsAnnotations:
        ...
        if id_ in clnged_ids:
            return clnged_ids[id_]
        ...
        if (
            ...
            newelem = elem._clnge(clnge=clnge, **kw)
        elif annotations != elem._annotations:
            if detect_subquery_cols and elem._is_immutable:
                to_annotate = elem._clnge(clnge=clnge, **kw)
            else:
                to_annotate = elem
        ...
        newelem._copy_internals(
            clnge=clnge, ind_cols_on_fromclause=ind_cols_on_fromclause
        )

        clnged_ids[id_] = newelem
        return newelem

    if element is not None:
        element = cast(_SA, clnge(element))
    clnge = None  # type: ignore  # remove gc cycles
    return element
...
def _deep_deannotate(
    ...
    clnged: Dict[Any, SupportsAnnotations] = {}
    ...
    def clnge(elem: SupportsAnnotations, **kw: Any) -> SupportsAnnotations:
        ...
        if key not in clnged:
            newelem = elem._deannotate(values=values, clnge=True)
            newelem._copy_internals(clnge=clnge)
            clnged[key] = newelem
            return newelem
        else:
            return clnged[key]
