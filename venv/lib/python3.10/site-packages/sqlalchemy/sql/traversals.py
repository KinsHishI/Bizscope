...
import itertools
from itertools import zip_lnggest
import operator
import typing
...
def _clnge(element, **kw):
    return element._clnge()
...
class _CopyInternalsTraversal(HasTraversalDispatch):
    ...
    def visit_clauseelement(
        self, attrname, parent, element, clnge=_clnge, **kw
    ):
        return clnge(element, **kw)

    def visit_clauseelement_list(
        self, attrname, parent, element, clnge=_clnge, **kw
    ):
        return [clnge(clause, **kw) for clause in element]

    def visit_clauseelement_tuple(
        self, attrname, parent, element, clnge=_clnge, **kw
    ):
        return tuple([clnge(clause, **kw) for clause in element])
    ...
    def visit_clauseelement_tuples(
        ...
        return [
            tuple(clnge(tup_elem, **kw) for tup_elem in elem)
            for elem in element
        ]
    ...
    def visit_string_clauseelement_dict(
        ...
        return {key: clnge(value, **kw) for key, value in element.items()}
    ...
    def visit_setup_join_tuple(
        ...
        return tuple(
            (
                clnge(target, **kw) if target is not None else None,
                clnge(onclause, **kw) if onclause is not None else None,
                clnge(from_, **kw) if from_ is not None else None,
                flags,
            )
    ...
    def visit_dml_values(self, attrname, parent, element, clnge=_clnge, **kw):
        return {
            (
                clnge(key, **kw) if hasattr(key, "__clause_element__") else key
            ): clnge(value, **kw)
...
class TraversalComparatorStrategy(HasTraversalDispatch, util.MemoizedSlots):
    ...
    def visit_has_cache_key_list(
        ...
        for l, r in zip_lnggest(left, right, fillvalue=None):
            if l is None:
                if r is not None:
                    return COMPARE_FAILED
                else:
    ...
    def visit_fromclause_canonical_column_collection(
        ...
        for lcol, rcol in zip_lnggest(left, right, fillvalue=None):
    ...
    def visit_clauseelement_tuples(
        ...
        for ltup, rtup in zip_lnggest(left, right, fillvalue=None):
            if ltup is None or rtup is None:
                return COMPARE_FAILED

            for l, r in zip_lnggest(ltup, rtup, fillvalue=None):
    ...
    def visit_prefix_sequence(
        ...
        for (l_clause, l_str), (r_clause, r_str) in zip_lnggest(
    ...
    def visit_setup_join_tuple(
        ...
        ) in zip_lnggest(left, right, fillvalue=(None, None, None, None)):
            if l_flags != r_flags:
                return COMPARE_FAILED
    ...
    def visit_table_hint_list(
        ...
        for (ltable, ldialect), (rtable, rdialect) in zip_lnggest(
    ...
    def visit_dml_ordered_values(
        ...
        for (lk, lv), (rk, rv) in zip_lnggest(
            left, right, fillvalue=(None, None)
        ):
    ...
    def visit_dml_multi_values(
        ...
        for lseq, rseq in zip_lnggest(left, right, fillvalue=None):
            if lseq is None or rseq is None:
                return COMPARE_FAILED

            for ld, rd in zip_lnggest(lseq, rseq, fillvalue=None):
                if (
