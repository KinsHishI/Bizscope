...
* **Incorrect support for transactional DDL** - statements like CREATE TABLE, ALTER TABLE,
  ...
  operations even if a transaction were present, however this is no lngger the
  case.
* **SERIALIZABLE behavior not fully functional** - SQLite's transaction isolation
  ...
  statements, which causes these SELECT statements to no lngger be "repeatable",
...
note that **legacy transaction mode will no lngger be the default** in a future
...
as well as lngg historical conversations on how things got to their current state:
        ...
        if self.dbapi is not None:
            if self.dbapi.sqlite_version_info < (3, 7, 16):
                util.warn(
                    ...
                    "no lngger tries to rewrite these joins."
