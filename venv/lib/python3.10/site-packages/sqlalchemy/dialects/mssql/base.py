...
.. deprecated:: 1.4
   ...
   These options are also no lngger returned as part of the
...
above ``Numeric(10, 0)`` to return Python ints (which also support "lngg"
...
.. versionchanged:: 2.0 The SQL Server dialect will no lngger implicitly
...
LIMIT and OFFSET, or just OFFSET alnge, will be rendered using the
...
* When this flag is ``False``, the :class:`.UnicodeText`,
  ...
  respectively.  This is the lngg-standing behavior of these types.
...
the driver still has this limitation, the ORM Versioning feature is no lngger
...
applications to have lngg held locks and frequent deadlocks.
