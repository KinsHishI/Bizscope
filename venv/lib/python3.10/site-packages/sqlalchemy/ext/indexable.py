...
as a dedicated attribute which behaves like a standalnge column::
...
structure is also automatically tracked as history; we no lngger need
...
* For an :class:`.index_property` that is given an integer index value,
  ...
  at least as lngg as the index value; the value is then set at its
