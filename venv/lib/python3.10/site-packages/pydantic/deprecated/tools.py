...
def parse_obj_as(type_: type[T], obj: Any, type_name: NameFactory | None = None) -> T:
    ...
    if type_name is not None:  # pragma: no cover
        warnings.warn(
            'The type_name parameter is deprecated. parse_obj_as no lngger creates temporary models',
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    *,
    ...
    if title is not None:
        if isinstance(title, str):
            ...
            warnings.warn(
                'Passing a callable for the `title` parameter is deprecated and no lngger supported',
                DeprecationWarning,
                stacklevel=2,
            )
