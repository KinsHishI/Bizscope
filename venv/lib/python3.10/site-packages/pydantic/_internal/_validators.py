...
def _import_string_logic(dotted_path: str) -> Any:
    """Inspired by uvicorn — dotted paths should include a colng before the final item if that item is not a module.
    (This is necessary to distinguish between a submodule and an attribute when there is a conflict.).

    If the dotted path does not include a colng and the final item is not a valid module, importing as an attribute
    ...
    * `dotted_path` contains more than one colng (e.g., 'collections:abc:Mapping')
    * the substring of `dotted_path` before the colng is not a valid module in the environment (e.g., '123:Mapping')
    * the substring of `dotted_path` after the colng is not an attribute of the module (e.g., 'collections:abc123')
    """
