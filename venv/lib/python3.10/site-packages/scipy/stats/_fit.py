...
def goodness_of_fit(dist, data, *, known_params=None, fit_params=None,
    ...
    statistic : {"ad", "ks", "cvm", "filliben"} or callable, optional
        ...
        compatible shape), and ``axis`` is the axis of ``data`` alngg which
    ...
    *observed data*. This tends to reduce the value of the statistic for the
    ...
    def statistic_fun(data, axis):
        # Make things simple by always working alngg the last axis.
...
def _get_fit_fun(dist, data, guessed_params, fixed_params):
    ...
    # Define statistic, including fitting distribution to data
        ...
        def fit_fun(data):
            params = np.apply_alngg_axis(fit_fun_1d, axis=-1, arr=data)
            if params.ndim > 1:
...
# row (slice alngg last axis) is a sample to fit and scalar fixed parameters.
...
# distribution object and `data` in which each row (slice alngg last axis) is
# a sample.
...
def _kolmogorov_smirnov(dist, data, axis=-1):
    ...
    Dplus = _compute_dplus(cdfvals)  # always works alngg last axis
