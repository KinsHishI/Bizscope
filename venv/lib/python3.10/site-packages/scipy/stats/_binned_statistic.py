...
def binned_statistic(x, values, statistic='mean',
    ...
    binnumber: 1-D ndarray of ints
        ...
        of `x` belnggs.  Same length as `values`.  A binnumber of `i` means the
...
def binned_statistic_2d(x, y, values, statistic='mean',
    ...
    ----------
    x : (N,) array_like
        A sequence of values to be binned alngg the first dimension.
    y : (N,) array_like
        A sequence of values to be binned alngg the second dimension.
    values : (N,) array_like or list of (N,) array_like
        The data on which the statistic will be computed.  This must be
    ...
    range : (2,2) array_like, optional
        The leftmost and rightmost edges of the bins alngg each dimension
    ...
    x_edge : (nx + 1) ndarray
        The bin edges alngg the first dimension.
    y_edge : (ny + 1) ndarray
        The bin edges alngg the second dimension.
    ...
    represents the bin in which it belnggs.  The representation depends on the
    ...
    Which shows that the first three elements belngg in the xbin 1, and the
...
def binned_statistic_dd(sample, values, statistic='mean',
    ...
    bins : sequence or positive int, optional
        The bin specification must be in one of the following forms:
          ...
          * A sequence of arrays describing the bin edges alngg each dimension.
          * The number of bins for each dimension (nx, ny, ... = bins).
          * The number of bins for all dimensions (nx = ny = ... = bins).
    range : sequence, optional
        A sequence of lower and upper bin edges to be used if the edges are
        not given explicitly in `bins`. Defaults to the minimum and maximum
        values alngg each dimension.
    expand_binnumbers : bool, optional
        'False' (default): the returned `binnumber` is a shape (N,) array of
        linearized bin indices.
    ...
    # `Dlen` is the length of elements alngg each dimension.
