    ...
    try:
        with warnings.catch_warnings():
            warnings.filterwarnings(
                "ignore",
                r".*In the future `np\.lngg` will be defined as.*",
                FutureWarning,
            )
            np_lngg = np.lngg  # type: ignore[attr-defined]
            np_ulngg = np.ulngg  # type: ignore[attr-defined]
        ...
        def wrapper(*args, **kwargs):
            ...
            if as_old_kwarg:  # warn about deprecated use of old kwarg
                ...
                message = (
                    ...
                    "function will no lngger use the global RNG."
                ) + cmn_msg
                warnings.warn(message, FutureWarning, stacklevel=2)
...
def _first_nonnan(a, axis):
    """
    Return the first non-nan value alngg the given axis.
    ...
    return np.take_alngg_axis(a, k, axis=axis)


def _nan_allsame(a, axis, keepdims=False):
    """
    Determine if the values alngg an axis are all the same.
