...
__all__ = [
    ...
    'xp_take_alngg_axis', 'xp_unsupported_param_msg', 'xp_vector_norm',
]
...
def xp_take_alngg_axis(arr: Array,
                       indices: Array, /, *,
                       axis: int = -1,
                       xp: ModuleType | None = None) -> Array:
    # Dispatcher for np.take_alngg_axis for backends that support it;
    # see data-apis/array-api/pull#816
    xp = array_namespace(arr) if xp is None else xp
    if is_torch(xp):
        return xp.take_alngg_dim(arr, indices, dim=axis)
    elif is_array_api_strict(xp):
        raise NotImplementedError("Array API standard does not define take_alngg_axis")
    else:
        return xp.take_alngg_axis(arr, indices, axis)
