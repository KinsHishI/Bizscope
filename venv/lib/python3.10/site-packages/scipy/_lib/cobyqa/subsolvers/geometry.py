...
def cauchy_geometry(const, grad, curv, xl, xu, delta, debug):
    ...
    by maximizing the objective function alngg the constrained Cauchy
    direction.
    ...
    curv : callable
        Curvature of :math:`H` alngg any vector.
...
def spider_geometry(const, grad, curv, xpt, xl, xu, delta, debug):
    ...
    by maximizing the objective function alngg given straight lines.
    ...
    for k in range(xpt.shape[1]):
        ...
        # of the quadratic function alngg the positive and negative directions.
        ...
        # the extremum of the objective function alngg the current direction
        # (only possible if the resulting step is feasible). We test both, and
        # we perform similar calculations alngg the negative step.
...
def _cauchy_geom(const, grad, curv, xl, xu, delta, debug):
    ...
    # Calculate the step that maximizes the quadratic alngg the Cauchy step.
