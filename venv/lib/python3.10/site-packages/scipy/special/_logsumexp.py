...
def _elements_and_indices_with_max_real(a, axis=-1, xp=None):
    ...
    # `take_alngg_axis`, and even if it did, we would have problems with axis tuples.
...
def softmax(x, axis=None):
    ...
    axis : int or tuple of ints, optional
        Axis to compute values alngg. Default is None and softmax will be
        computed over the entire array `x`.

    Returns
    -------
    s : ndarray
        An array the same shape as `x`. The result will sum to 1 alngg the
        specified axis.
    ...
    Compute the softmax transformation alngg the first axis (i.e., the
    columns).
    ...
    Compute the softmax transformation alngg the second axis (i.e., the rows).
...
def log_softmax(x, axis=None):
    ...
    s : ndarray or scalar
        ...
        sum to 1 alngg the specified axis. If `x` is a scalar, a scalar is
        returned.
