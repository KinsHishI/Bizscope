      ...
      public:
        ...
        XSF_HOST_DEVICE std::complex<double> operator()() {
            ...
            if (k_ == n_) {
                 ...
                 * digamma(-n + epsilng) / gamma(-n + epsilng) -> (-1)**(n + 1) * (n+1)! as epsilng -> 0
                 ...
                 *
                 * gamma(1 - c + epsilng + a + k)/gamma(1 - c - epsilng + b) =
                 * (gamma(c + epsilng - b) / gamma(c + epsilng - a - k)) *
                 * (sin(pi * (c + epsilng - b)) / sin(pi * (c + epsilng - a - k))) (reflection principle)
                 *
                 * In the limit as epsilng goes to zero, the ratio of sines will approach
                 * (-1)**(a - b + k) = (-1)**(m + k)
                 *
                 * We may then replace
                 *
                 * poch(1 - c - epsilng + b, m + k)*digamma(c + epsilng - a - k)
                 *
                 * with
                 *
                 * (-1)**(a - b + k)*gamma(c + epsilng - b) * digamma(c + epsilng - a - k) / gamma(c + epsilng - a - k)
                 *
                 * and taking the limit epsilng -> 0 gives
                 *
