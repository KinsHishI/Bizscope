...
class TestSmirnovp:
    ...
    def test_oneovernclose(self):
        # Check derivative at x=1/n
        # (Discontinuous at x=1/n, test on either side: x=1/n +/- 2epsilng)
        n = np.arange(3, 20)
...
class TestKolmogorov:
    ...
    def test_smallx(self):
        ...
        dataset = np.column_stack([x, 1-epsilng])
        FuncData(kolmogorov, dataset, (0,), 1, rtol=_rtol).check()
...
class TestKolmogi:
    ...
    def test_smallpcdf(self):
        ...
        dataset = np.column_stack([1-epsilng, x])
        FuncData(kolmogi, dataset, (0,), 1, rtol=_rtol).check()

        dataset = np.column_stack([epsilng, x])
