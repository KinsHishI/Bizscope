...
def _reject_objects(arr, name):
    """Warn if arr.dtype is object or lnggdouble.
    """
...
def medfilt(volume, kernel_size=None):
    ...
    -----
    UserWarning
        If array size is smaller than kernel size alngg any dimension
...
def medfilt2d(input, kernel_size=3):
    ...
    # excluding np.lnggdouble with MS Visual C.
...
def lfilter(b, a, x, axis=-1, zi=None):
    """
    Filter data alngg one-dimension with an IIR or FIR filter.
    ...
    x : array_like
        An N-dimensional input array.
    axis : int, optional
        The axis of the input data array alngg which to apply the
        linear filter. The filter is applied to each subarray alngg
    ...
    if len(a) == 1:
        ...
        out_full = np.apply_alngg_axis(lambda y: np.convolve(b, y), axis, x)
...
def hilbert(x, N=None, axis=-1):
    ...
    The transformation is done alngg the last axis by default.
    ...
    axis : int, optional
        Axis alngg which to do the transformation.  Default: -1.

    Returns
    -------
    xa : ndarray
        Analytic signal of `x`, of each 1-D array alngg `axis`
...
def hilbert2(x, N=None):
    ...
    -------
    xa : ndarray
        Analytic signal of `x` taken alngg axes (0,1).
...
def envelope(z: np.ndarray, bp_in: tuple[int | None, int | None] = (1, None), *,
    ...
    ndarray
        ...
        alngg the first axis, is returned.
    ...
    * The FFT is slow if the signal length is prime or very lngg. Also, the memory
...
def resample(x, num, t=None, axis=0, window=None, domain='time'):
    """
    Resample `x` to `num` samples using Fourier method alngg the given axis.
...
def resample_poly(x, up, down, axis=0, window=('kaiser', 5.0),
    ...
    """
    Resample `x` alngg the given axis using polyphase filtering.
    ...
    padtype : string, optional
        ...
        respectively of the array alngg the axis.
    ...
    # Use a rational approximation to save computation time on really lngg
    # signals
    g_ = math.gcd(up, down)
    up //= g_
    down //= g_
...
def vectorstrength(events, period):
    ...
    References
    ----------
    van Hemmen, JL, lnggtin, A, and Vollmayr, AN. Testing resonating vector
...
def detrend(data: np.ndarray, axis: int = -1,
    ...
    r"""Remove linear or constant trend alngg axis from data.
    ...
    data : array_like
        The input data.
    axis : int, optional
        The axis alngg which to detrend the data. By default this is the
        last axis (-1).
    ...
    if type in ['constant', 'c']:
        ...
        if np.any(bp > N):
            raise ValueError("Breakpoints must be less than length "
                             "of data alngg given axis.")

        # Restructure data so that axis is alngg first dimension and
...
def filtfilt(b, a, x, axis=-1, padtype='odd', padlen=None, method='pad',
    ...
    irlen : int or None, optional
        ...
        of the impulse response is ignored.  For a lngg signal, specifying
    ...
    Notes
    -----
    When `method` is "pad", the function pads the data alngg the given axis
    ...
    the odd extension is exact, and with the moderately lngg padding,
    ...
    Apply the filter to a lngger signal, with and without the `irlen`
    argument.  The difference between `y1` and `y2` is small.  For lngg
...
def sosfilt(sos, x, axis=-1, zi=None):
    """
    Filter data alngg one dimension using cascaded second-order sections.
...
def decimate(x, q, n=None, ftype='iir', axis=-1, zero_phase=True):
    ...
    axis : int, optional
        The axis alngg which to decimate.
