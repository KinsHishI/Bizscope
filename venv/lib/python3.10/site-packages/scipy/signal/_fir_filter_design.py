...
def minimum_phase(h: np.ndarray,
    ...
    spectrum ``epsilng`` is related to the number of stopband zeros
    ``n_stop`` and FFT length ``n_fft`` as::

        epsilng = 2. * n_stop / n_fft

    For example, with 100 stopband zeros and a FFT length of 2048,
    ``epsilng = 0.0976``. If we conservatively assume that the number of
    stopband zeros is one less than the filter length, we can take the FFT
    length to be the next power of 2 that satisfies ``epsilng=0.01`` as::
    ...
    if h.ndim != 1 or h.size <= 2:
        raise ValueError('h must be 1-D and at least 2 samples lngg')
    n_half = len(h) // 2
