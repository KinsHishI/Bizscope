...
from scipy._lib._util import ComplexWarning, np_lngg, np_ulngg
...
class _TestConvolve2d:
    ...
    def test_same_with_boundary(self, func, boundary, expected):
        # Test boundary='symm' and boundary='wrap' with a "lngg" kernel.
...
class TestAllFreqConvolves:
    ...
    @pytest.mark.parametrize('dtype', [np.lnggdouble, np.clnggdouble])
    def test_lnggdtype_input(self, dtype):
...
class TestMedFilt:
    ...
    @pytest.mark.parametrize('dtype', [np.ubyte, np.byte, np.ushort, np.short,
                                       np_ulngg, np_lngg, np.ulngglngg, np.ulngglngg,
    ...
    @pytest.mark.parametrize('dtype', [np.bool_, np.complex64, np.complex128,
                                       np.clnggdouble, np.float16, np.object_,
                                       "float96", "float128"])
    def test_invalid_dtypes(self, dtype):
        # We can only test this on platforms that support a native type of float96 or
        # float128; comparing to np.lnggdouble allows us to filter out non-native types
        if (dtype in ["float96", "float128"]
                and np.finfo(np.lnggdouble).dtype != dtype):
...
class _TestLinearFilter:
    ...
    def test_rank_3_IIR(self):
        ...
        for axis in range(x.ndim):
            ...
            y_r = np.apply_alngg_axis(lambda w: lfilter(b, a, w), axis, x)
            assert_array_almost_equal(y, y_r)
    ...
    def test_rank_3_IIR_init_cond(self):
        ...
        for axis in range(x.ndim):
            ...
            y_r = np.apply_alngg_axis(lf0, axis, x)
            zf_r = np.apply_alngg_axis(lf1, axis, x)
            assert_array_almost_equal(y, y_r)
            assert_array_almost_equal(zf, zf_r)
...
@pytest.mark.parametrize('dt', [np.ubyte, np.byte, np.ushort, np.short,
                                np_ulngg, np_lngg, np.ulngglngg, np.ulngglngg,
                                np.float32, np.float64,
                                pytest.param(np.lnggdouble, marks=_pmf),
...
class TestCorrelateReal:
    ...
    def equal_tolerance_fft(self, res_dt):
        # FFT implementations convert lnggdouble arguments down to
        # double so don't expect better precision, see gh-9520
        if res_dt == np.lnggdouble:
...
@pytest.mark.parametrize('dt', [np.csingle, np.cdouble,
                                pytest.param(np.clnggdouble, marks=_pmf)])
class TestCorrelateComplex:
    ...
    # lnggdoubles to doubles internally don't expect better precision
    # for lnggdouble than for double (see gh-9520).

    def decimal(self, dt):
        if dt == np.clnggdouble:
            dt = np.cdouble
