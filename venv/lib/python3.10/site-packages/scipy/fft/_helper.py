...
def next_fast_len(target, real=False):
    ...
    compute; 1.9 times lngger than the size given by ``next_fast_len``:
...
def _init_nd_shape_and_axes(x, shape, axes):
    ...
    axes : int or array_like of ints or None
        Axes alngg which the calculation is computed.
    ...
    axes : list
        Axes alngg which the calculation is computed, as a list of integers.
...
def fftshift(x, axes=None):
    ...
    Shift the zero-frequency component only alngg the second axis:
