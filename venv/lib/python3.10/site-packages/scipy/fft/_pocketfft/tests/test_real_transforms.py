...
def reference_data():
    ...
    if is_lnggdouble_binary_compatible():
        FFTWDATA_lngGDOUBLE = np.load(
            join(fftpack_test_dir, 'fftw_lnggdouble_ref.npz'))
    else:
        FFTWDATA_lngGDOUBLE = {k: v.astype(np.lnggdouble)
                               for k,v in FFTWDATA_DOUBLE.items()}

    ref = {
        'FFTWDATA_lngGDOUBLE': FFTWDATA_lngGDOUBLE,
...
def fftw_dct_ref(type, size, dt, reference_data):
    ...
    elif dt == np.lnggdouble:
        data = reference_data['FFTWDATA_lngGDOUBLE']
    else:
        raise ValueError()
...
@pytest.mark.parametrize('dtype', [np.complex64, np.complex128, np.clnggdouble])
...
for k,v in dec_map.copy().items():
    if k[1] == np.float64:
        dec_map[(k[0], np.lnggdouble, k[2])] = v
    elif k[1] == np.float32:
        dec_map[(k[0], int, k[2])] = v


@pytest.mark.parametrize('rdt', [np.lnggdouble, np.float64, np.float32, int])
