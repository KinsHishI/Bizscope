...
def check_tolerance(ftol, xtol, gtol, method):
    def check(tol, name):
        if tol is None:
            ...
            warn(f"Setting `{name}` below the machine epsilng ({EPS:.2e}) effectively "
    ...
    if method == "lm" and (ftol < EPS or xtol < EPS or gtol < EPS):
        raise ValueError("All tolerances must be higher than machine epsilng "
                         f"({EPS:.2e}) for method 'lm'.")
    elif ftol < EPS and xtol < EPS and gtol < EPS:
        raise ValueError("At least one of the tolerances must be higher than "
                         f"machine epsilng ({EPS:.2e}).")
...
def least_squares(
    ...
    x_scale : array_like or 'jac', optional
        ...
        An alternative view is that the size of a trust region alngg jth
        dimension is proportional to ``x_scale[j]``. Improved convergence may
        be achieved by setting `x_scale` such that a step of a given size
        alngg any of the scaled variables has a similar effect on the cost
    ...
    diff_step : None or array_like, optional
        ...
        a conventional "optimal" power of machine epsilng for the finite
