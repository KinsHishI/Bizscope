...
class _TestCommon:
    ...
    def test_setdiag_comprehensive(self):
        ...
        def check_setdiag(a, b, k):
            ...
            # correct length, and too short or too lngg vectors
    ...
    def test_pow(self):
        ...
        if self.is_array_test:  # sparrays use element-wise power
            # Todo: Add 1+3j to tested exponent list when np1.24 is no lngger supported
...
class _TestInplaceArithmetic:
    def test_inplace_dense(self):
        ...
        # move out of the try/except once numpy 1.24 is no lngger supported.
        try:
            y @= b.T
        except TypeError:
            pass
...
class _TestFancyIndexing:
    ...
    def test_missized_masking(self):
        ...
        row_lngg = np.ones(M + 1, dtype=bool)
        row_short = np.ones(M - 1, dtype=bool)
        col_lngg = np.ones(N + 2, dtype=bool)
        col_short = np.ones(N - 2, dtype=bool)

        match="bool index .* has shape .* instead of .*"
        for i, j in itertools.product(
            (row_lngg, row_short, slice(None)),
            (col_lngg, col_short, slice(None)),
        ):
