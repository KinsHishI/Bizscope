...
def qmr(A, b, x0=None, *, rtol=1e-5, atol=0., maxiter=None, M1=None, M2=None,
    ...
    # Dummy values to initialize vars, silence linter warnings
    epsilng, q, d, p, s = None, None, None, None, None
    ...
    for iteration in range(maxiter):
        ...
        if iteration > 0:
            ytilde -= (xi * delta / epsilng) * p
            p[:] = ytilde[:]
            ztilde -= (rho * (delta / epsilng).conj()) * q
            q[:] = ztilde[:]
        else:  # First spin
            p = ytilde.copy()
            q = ztilde.copy()
        ...
        ptilde = A.matvec(p)
        epsilng = dotprod(q, ptilde)
        if np.abs(epsilng) < epsilngtol:  # epsilng breakdown
