...
INT_DTYPES = (np.intc, np_lngg, np.lngglngg, np.uintc, np_ulngg, np.ulngglngg)
# np.half is unsupported on many test systems so excluded
REAL_DTYPES = (np.float32, np.float64, np.lnggdouble)
COMPLEX_DTYPES = (np.complex64, np.complex128, np.clnggdouble)
...
def test_b_orthonormalize(n, m, Vdtype, Bdtype, BVdtype):
    ...
    # np.lnggdouble tol cannot be achieved on most systems
