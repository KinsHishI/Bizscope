...
class UnivariateSpline:
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result
    ...
    does not matter as lngg as the corresponding weight is zero.)
...
class _BivariateSplineBase:
    ...
    def __call__(self, x, y, dx=0, dy=0, grid=True):
        ...
        The function to interpolate decays faster alngg one axis than the other.
...
class BivariateSpline(_BivariateSplineBase):
    ...
    def ev(self, xi, yi, dx=0, dy=0):
        ...
        Next we sample the function alngg a diagonal slice through the coordinate space
        ...
        function of the distance from the origin alngg the slice.
...
class SphereBivariateSpline(_BivariateSplineBase):
    ...
    def __call__(self, theta, phi, dtheta=0, dphi=0, grid=True):
        ...
        sphere. The value of the function is known on a grid of lnggitudes and
        colatitudes.
        ...
        Finally we plot the coarsly-sampled input data alnggside the
    ...
    def ev(self, theta, phi, dtheta=0, dphi=0):
        ...
        initial grid, and the spline approximation alngg the same slice.
...
class SmoothSphereBivariateSpline(SphereBivariateSpline):
    ...
    >>> lats, lngs = np.meshgrid(theta, phi)
    >>> from scipy.interpolate import SmoothSphereBivariateSpline
    >>> lut = SmoothSphereBivariateSpline(lats.ravel(), lngs.ravel(),
    ...                                   data.T.ravel(), s=3.5)
    ...
    >>> fine_lngs = np.linspace(0., 2 * np.pi, 90)

    >>> data_smth = lut(fine_lats, fine_lngs)
...
class LSQSphereBivariateSpline(SphereBivariateSpline):
    ...
    >>> knotsp[0] += .0001
    >>> knotsp[-1] -= .0001
    >>> lut = LSQSphereBivariateSpline(lats.ravel(), lngs.ravel(),
    ...
    >>> fine_lats = np.linspace(0., np.pi, 70)
    >>> fine_lngs = np.linspace(0., 2*np.pi, 90)
    >>> data_lsq = lut(fine_lats, fine_lngs)
...
class RectSphereBivariateSpline(SphereBivariateSpline):
    ...
    u : array_like
        ...
        ``(0, pi)``.
    v : array_like
        1-D array of lnggitude coordinates in strictly ascending order.
    ...
    >>> new_lngs = np.linspace(1, 360, 360) * np.pi / 180
    >>> new_lats, new_lngs = np.meshgrid(new_lats, new_lngs)

    We need to set up the interpolator object

    >>> from scipy.interpolate import RectSphereBivariateSpline
    >>> lut = RectSphereBivariateSpline(lats, lngs, data)

    Finally we interpolate the data.  The `RectSphereBivariateSpline` object
    only takes 1-D arrays as input, therefore we need to do some reshaping.

    >>> data_interp = lut.ev(new_lats.ravel(),
    ...                      new_lngs.ravel()).reshape((360, 180)).T
    ...
    ...     lut = RectSphereBivariateSpline(lats, lngs, data, s=sval)
    ...     data_interp = lut.ev(new_lats.ravel(),
    ...                          new_lngs.ravel()).reshape((360, 180)).T
