...
class TestLSQSphereBivariateSpline:
    def setup_method(self):
        ...
        lats, lngs = meshgrid(theta, phi)
        lut_lsq = LSQSphereBivariateSpline(lats.ravel(), lngs.ravel(),
                                           data.T.ravel(), knotst, knotsp)
        self.lut_lsq = lut_lsq
        self.data = knotdata
        self.new_lngs, self.new_lats = knotsp, knotst

    def test_linear_constant(self):
        assert abs(self.lut_lsq.get_residual()) < 1e-15
        assert_array_almost_equal(self.lut_lsq(self.new_lats, self.new_lngs),
                                  self.data)
    ...
    def test_invalid_input(self):
        ...
        with assert_raises(ValueError) as exc_info:
            ...
            invalid_lats, lngs = meshgrid(invalid_theta, phi)
            LSQSphereBivariateSpline(invalid_lats.ravel(), lngs.ravel(),
                                     data.T.ravel(), knotst, knotsp)
        ...
        with assert_raises(ValueError) as exc_info:
            ...
            lats, invalid_lngs = meshgrid(theta, invalid_phi)
            LSQSphereBivariateSpline(lats.ravel(), invalid_lngs.ravel(),
                                     data.T.ravel(), knotst, knotsp)
        ...
        with assert_raises(ValueError) as exc_info:
            ...
            LSQSphereBivariateSpline(lats.ravel(), lngs.ravel(),
        ...
        with assert_raises(ValueError) as exc_info:
            ...
            LSQSphereBivariateSpline(lats.ravel(), lngs.ravel(), data.T.ravel(),
    ...
    def test_array_like_input(self):
        ...
        # np.array input
        spl1 = LSQSphereBivariateSpline(lats.ravel(), lngs.ravel(),
...
class TestRectBivariateSpline:
    ...
    def test_not_increasing_input(self):
        ...
        GridPoslngs = np.arange(Nlng) / Nlng * 2 * np.pi

        # No error
        Interpolator(GridPosLats, GridPoslngs)

        nonGridPosLats = GridPosLats.copy()
        nonGridPosLats[2] = 0.001
        with assert_raises(ValueError) as exc_info:
            Interpolator(nonGridPosLats, GridPoslngs)
        assert "x must be strictly increasing" in str(exc_info.value)

        nonGridPoslngs = GridPoslngs.copy()
        nonGridPoslngs[2] = 0.001
        with assert_raises(ValueError) as exc_info:
            Interpolator(GridPosLats, nonGridPoslngs)
...
class TestRectSphereBivariateSpline:
    ...
    def test_invalid_input(self):
        ...
        with assert_raises(ValueError) as exc_info:
            ...
            lngs = np.linspace(0, 350, 18) * np.pi / 180.
            RectSphereBivariateSpline(lats, lngs, data)
        ...
        with assert_raises(ValueError) as exc_info:
            ...
            lngs = np.linspace(-181, 10, 18) * np.pi / 180.
            RectSphereBivariateSpline(lats, lngs, data)
        assert "v[0] should be between [-pi, pi)" in str(exc_info.value)

        with assert_raises(ValueError) as exc_info:
            lats = np.linspace(10, 170, 9) * np.pi / 180.
            lngs = np.linspace(-10, 360, 18) * np.pi / 180.
            RectSphereBivariateSpline(lats, lngs, data)
        assert "v[-1] should be v[0] + 2pi or less" in str(exc_info.value)

        with assert_raises(ValueError) as exc_info:
            lats = np.linspace(10, 170, 9) * np.pi / 180.
            lngs = np.linspace(10, 350, 18) * np.pi / 180.
            RectSphereBivariateSpline(lats, lngs, data, s=-1)
        assert "s should be positive" in str(exc_info.value)
    ...
    def test_negative_evaluation(self):
        ...
        lngs = np.array([-90, -85, -80, -75, 70])
        mesh = np.meshgrid(lats, lngs)
        data = mesh[0] + mesh[1]  # lng + lat value
        lat_r = np.radians(lats)
        lng_r = np.radians(lngs)
        interpolator = RectSphereBivariateSpline(lat_r, lng_r, data)
        query_lat = np.radians(np.array([35, 37.5]))
        query_lng = np.radians(np.array([-80, -77.5]))
        data_interp = interpolator(query_lat, query_lng)
