...
class _TestRBFInterpolator:
    ...
    def test_scale_invariance_1d(self, kernel):
        ...
        yitp1 = self.build(x, y, epsilng=1.0, kernel=kernel)(xitp)
        yitp2 = self.build(x, y, epsilng=2.0, kernel=kernel)(xitp)
    ...
    def test_extreme_domains(self, kernel):
        ...
        if kernel in _SCALE_INVARIANT:
            ...
            yitp1 = self.build(x, y, epsilng=5.0, kernel=kernel)(xitp)
            yitp2 = self.build(
                x*scale + shift, y,
                epsilng=5.0/scale,
                kernel=kernel
    ...
    def test_interpolation_misfit_1d(self, kernel):
        ...
        # appropriate `epsilng`, does a good job at interpolation in 1d.
        seq = Halton(1, scramble=False, seed=np.random.RandomState())
        ...
        yitp = self.build(x, y, epsilng=5.0, kernel=kernel)(xitp)
    ...
    def test_interpolation_misfit_2d(self, kernel):
        # Make sure that each kernel, with its default `degree` and an
        # appropriate `epsilng`, does a good job at interpolation in 2d.
        seq = Halton(2, scramble=False, seed=np.random.RandomState())
    ...
    def test_epsilng_not_specified_error(self):
        y = np.linspace(0, 1, 5)[:, None]
        d = np.zeros(5)
        for kernel in _AVAILABLE:
            ...
            match = '`epsilng` must be specified'
            with pytest.raises(ValueError, match=match):
        ...
        for kernel, deg in _NAME_TO_MIN_DEGREE.items():
            ...
            if deg >= 1:
                ...
                with pytest.warns(Warning, match=match):
                    self.build(y, d, epsilng=1.0, kernel=kernel, degree=deg-1)
    ...
    def test_minus_one_degree(self):
        ...
        for kernel, _ in _NAME_TO_MIN_DEGREE.items():
            self.build(y, d, epsilng=1.0, kernel=kernel, degree=-1)
