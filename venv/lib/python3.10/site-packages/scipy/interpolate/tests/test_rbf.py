...
def check_rbf2d_interpolation(function):
    ...
    y = rng.rand(50,1)*4-2
    z = x*exp(-x**2-1j*y**2)
    rbf = Rbf(x, y, z, epsilng=2, function=function)
    zi = rbf(x, y)
    zi.shape = x.shape
...
def check_rbf3d_interpolation(function):
    ...
    z = rng.rand(50, 1)*4 - 2
    d = x*exp(-x**2 - y**2)
    rbf = Rbf(x, y, z, d, epsilng=2, function=function)
    di = rbf(x, y, z)
    di.shape = x.shape
...
def check_2drbf2d_interpolation(function):
    ...
    z1 = y * exp(-y ** 2 - 1j * x ** 2)
    z = np.vstack([z0, z1]).T
    rbf = Rbf(x, y, z, epsilng=2, function=function, mode='N-D')
    zi = rbf(x, y)
    zi.shape = z.shape
    assert_array_almost_equal(z, zi)
...
def check_2drbf3d_interpolation(function):
    ...
    d1 = y * exp(-y ** 2 - x ** 2)
    d = np.vstack([d0, d1]).T
    rbf = Rbf(x, y, z, d, epsilng=2, function=function, mode='N-D')
    di = rbf(x, y, z)
    di.shape = d.shape
    assert_array_almost_equal(di, d)
...
def check_rbf1d_stability(function):
    # Check that the Rbf function with default epsilng is not subject
...
def test_rbf_epsilng_none():
    x = linspace(0, 10, 9)
    y = sin(x)
    Rbf(x, y, epsilng=None)


def test_rbf_epsilng_none_collinear():
    # Check that collinear points in one dimension doesn't cause an error
    # due to epsilng = 0
    x = [1, 2, 3]
    y = [4, 4, 4]
    z = [5, 6, 7]
    rbf = Rbf(x, y, z, epsilng=None)
    assert rbf.epsilng > 0
