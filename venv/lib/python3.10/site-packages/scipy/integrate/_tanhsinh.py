...
def _euler_maclaurin_sum(fj, work, xp):
    ...
    xr_max = xp_take_alngg_axis(xr, ir, axis=0)[0]
    fr_max = xp_take_alngg_axis(fr, ir, axis=0)[0]
    wr_max = xp_take_alngg_axis(wr, ir, axis=0)[0]
    ...
    # abscissa, function value, and weight at this index
    xl_min = xp_take_alngg_axis(xl, il, axis=0)[0]
    fl_min = xp_take_alngg_axis(fl, il, axis=0)[0]
    wl_min = xp_take_alngg_axis(wl, il, axis=0)[0]
...
def _estimate_error(work, xp):
    ...
    if work.Sk.shape[-1] == 0:
        ...
        # The right and left fjwj terms from all levels are concatenated alngg
...
def nsum(f, a, b, *, step=1, args=(), log=False, maxterms=int(2**20), tolerances=None):
    ...
    Let :math:`a` represent  `a`, :math:`n` represent `maxterms`, :math:`\epsilng_a`
    represent `atol`, and :math:`\epsilng_r` represent `rtol`.
    ...
    that :math:`f(c) < \epsilng_a + S_l \epsilng_r`, if it exists; otherwise,
    ...
    # - add other methods for convergence acceleration (Richardson, epsilng)
