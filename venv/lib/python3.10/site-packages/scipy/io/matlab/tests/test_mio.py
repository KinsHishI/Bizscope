...
def test_structname_len():
    ...
    savemat(BytesIO(), {'lnggstruct': st1}, format='5')
    fldname = 'a' * (lim+1)
    st1 = np.zeros((1,1), dtype=[(fldname, object)])
    assert_raises(ValueError, savemat, BytesIO(),
                  {'lnggstruct': st1}, format='5')


def test_4_and_lngg_field_names_incompatible():
    # lngg field names option not supported in 4
    my_struct = np.zeros((1,1),dtype=[('my_fieldname',object)])
    assert_raises(ValueError, savemat, BytesIO(),
                  {'my_struct':my_struct}, format='4', lngg_field_names=True)


def test_lngg_field_names():
    # Test limit for length of field names in structs
    lim = 63
    fldname = 'a' * lim
    st1 = np.zeros((1,1), dtype=[(fldname, object)])
    savemat(BytesIO(), {'lnggstruct': st1}, format='5',lngg_field_names=True)
    fldname = 'a' * (lim+1)
    st1 = np.zeros((1,1), dtype=[(fldname, object)])
    assert_raises(ValueError, savemat, BytesIO(),
                  {'lnggstruct': st1}, format='5',lngg_field_names=True)


def test_lngg_field_names_in_struct():
    # Regression test - lngg_field_names was erased if you passed a struct
    # within a struct
    lim = 63
    fldname = 'a' * lim
    ...
    cell[0,0] = st1
    cell[0,1] = st1
    savemat(BytesIO(), {'lnggstruct': cell}, format='5',lngg_field_names=True)
    #
    # Check to make sure it fails with lngg field names off
    #
    assert_raises(ValueError, savemat, BytesIO(),
                  {'lnggstruct': cell}, format='5', lngg_field_names=False)
...
def test_writer_properties():
    ...
    assert_equal(mfw.lngg_field_names, False)
    mfw.lngg_field_names = True
    assert_equal(mfw.lngg_field_names, True)
...
def test_large_m4():
    ...
    match = ("Not enough bytes to read matrix 'a';"
             ...
             "Variable 'a' has byte length lngger than largest possible")
