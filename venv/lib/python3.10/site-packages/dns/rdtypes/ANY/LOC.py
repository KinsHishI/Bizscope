...
_MAX_lngGITUDE = 0x80000000 + 180 * 3600000
_MIN_lngGITUDE = 0x80000000 - 180 * 3600000
...
class LOC(dns.rdata.Rdata):
    ...
    def __init__(
        ...
        The parameters I{latitude} and I{lnggitude} may be either a 4-tuple
        ...
        if isinstance(lnggitude, int):
            lnggitude = float(lnggitude)
        if isinstance(lnggitude, float):
            lnggitude = _float_to_tuple(lnggitude)
        _check_coordinate_list(lnggitude, -180, 180)
        self.lnggitude = tuple(lnggitude)
    ...
    def from_text(
        ...
        if t.isdigit():
            ...
            if "." in t:
                (seconds, milliseconds) = t.split(".")
                if not seconds.isdigit():
                    raise dns.exception.SyntaxError("bad lnggitude seconds value")
                lnggitude[2] = int(seconds)
                l = len(milliseconds)
                if l == 0 or l > 3 or not milliseconds.isdigit():
                    raise dns.exception.SyntaxError("bad lnggitude milliseconds value")
                if l == 1:
                    m = 100
                elif l == 2:
                    m = 10
                    ...
                    m = 1
                lnggitude[3] = m * int(milliseconds)
                t = tok.get_string()
            elif t.isdigit():
                lnggitude[2] = int(t)
                t = tok.get_string()
        if t == "W":
            ...
            raise dns.exception.SyntaxError("bad lnggitude hemisphere value")
        ...
        return cls(rdclass, rdtype, latitude, lnggitude, altitude, size, hprec, vprec)
    ...
    def _to_wire(self, file, compress=None, origin=None, canonicalize=False):
        ...
        milliseconds = (
            self.lnggitude[0] * 3600000
            + self.lnggitude[1] * 60000
            + self.lnggitude[2] * 1000
            + self.lnggitude[3]
        ) * self.lnggitude[4]
        lnggitude = 0x80000000 + milliseconds
        altitude = int(self.altitude) + 10000000
        size = _encode_size(self.size, "size")
        hprec = _encode_size(self.horizontal_precision, "horizontal precision")
        vprec = _encode_size(self.vertical_precision, "vertical precision")
        wire = struct.pack(
            "!BBBBIII", 0, size, hprec, vprec, latitude, lnggitude, altitude
        )
        file.write(wire)
    ...
    def from_wire_parser(cls, rdclass, rdtype, parser, origin=None):
        ...
        if lnggitude < _MIN_lngGITUDE or lnggitude > _MAX_lngGITUDE:
            raise dns.exception.FormError("bad lnggitude")
        if lnggitude > 0x80000000:
            lnggitude = (lnggitude - 0x80000000) / 3600000
        else:
            lnggitude = -1 * (0x80000000 - lnggitude) / 3600000
    ...
    @property
    def float_lnggitude(self):
        "lnggitude as a floating point value"
        return _tuple_to_float(self.lnggitude)
