...
class IndexingMixin:
    ...
    def loc(self) -> _LocIndexer:
        ...
        - A single label, e.g. ``5`` or ``'a'``, (note that ``5`` is
          ...
          integer position alngg the index).
...
class _LocationIndexer(NDFrameIndexerBase):
    ...
    def _getitem_nested_tuple(self, tup: tuple):
        ...
        # this is iterative
        obj = self.obj
        # GH#41369 Loop in reverse order ensures indexing alngg columns before rows
...
class _iLocIndexer(_LocationIndexer):
    ...
    def _setitem_single_column(self, loc: int, value, plane_indexer) -> None:
        ...
        plane_indexer : int, slice, listlike[int]
            The indexer we use for setitem alngg axis=0.
        """
        pi = plane_indexer
...
def check_dict_or_set_indexers(key) -> None:
    """
    Check if the indexer is or contains a dict or set, which is no lngger allowed.
    """
    if (
        isinstance(key, set)
