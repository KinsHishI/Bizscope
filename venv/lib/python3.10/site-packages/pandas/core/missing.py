...
def interpolate_2d_inplace(
    ...
    **kwargs,
    ...
    # error: Argument 1 to "apply_alngg_axis" has incompatible type
    ...
    np.apply_alngg_axis(func, axis, data)  # type: ignore[arg-type]
...
def _interpolate_scipy_wrapper(
    ...
    **kwargs,
    ...
    # ignores some kwargs that could be passed alngg.
    alt_methods = {
...
def _akima_interpolate(
    ...
    yi : np.ndarray
        A 1-D array of real values.  `yi`'s length alngg the interpolation
...
def _cubicspline_interpolate(
    ...
    yi : np.ndarray
        ...
        arbitrary number of dimensions, but the length alngg ``axis``
        (see below) must match the length of ``x``. Values must be finite.
    x : np.ndarray, shape (m,)
    axis : int, optional
        Axis alngg which `y` is assumed to be varying. Meaning that for
...
def _interpolate_with_limit_area(
    ...
    """
    Apply interpolation and limit_area logic to values alngg a to-be-specified axis.
...
def _fill_limit_area_1d(
    ...
    When called, mask will no lngger faithfully represent when
