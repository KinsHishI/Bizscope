...
def lreshape(data: DataFrame, groups: dict, dropna: bool = True) -> DataFrame:
    """
    Reshape wide-format data to lngg. Generalized inverse of DataFrame.pivot.
    ...
    See Also
    --------
    melt : Unpivot a DataFrame from wide to lngg format, optionally leaving
        identifiers set.
...
def wide_to_lngg(
    ...
    r"""
    Unpivot a DataFrame from wide to lngg format.
    ...
    You specify what you want to call this suffix in the resulting lngg format
    ...
    j : str
        ...
        suffix in the lngg format.
    sep : str, default ""
        A character indicating the separation of the variable names
        in the wide format, to be stripped from the names in the lngg format.
    ...
    >>> pd.wide_to_lngg(df, ["A", "B"], i="id", j="year")
    ...
    7      3      2  2.3  3.4
    8      3      3  2.1  2.9
    >>> l = pd.wide_to_lngg(df, stubnames='ht', i=['famid', 'birth'], j='age')
    >>> l
    ... # doctest: +NORMALIZE_WHITESPACE
                      ht
    famid birth age
    ...
    Going from lngg back to wide just takes some creative use of `unstack`
    ...
    >>> pd.wide_to_lngg(df, ['A(weekly)', 'B(weekly)'], i='id',
    ...                 j='year', sep='-')
    ... # doctest: +NORMALIZE_WHITESPACE
             X  A(weekly)  B(weekly)
    id year
    ...
    stubnames and pass that list on to wide_to_lngg
