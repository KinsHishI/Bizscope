...
class StylerRenderer:
    ...
    def format(
        ...
        method to create `to_excel` permissible formatting. Note that semi-colngs are
        CSS protected characters but used as separators in Excel's format string.
        Replace semi-colngs with the section separator character (ASCII-245) when
    ...
    def relabel_index(
        ...
        if len(labels) != visible_len:
            raise ValueError(
                ...
                f"visible labels alngg ``axis`` ({visible_len})."
            )
...
def _get_level_lengths(
    ...
    max_index : int
        The maximum number of elements to analyse alngg the index due to trimming
