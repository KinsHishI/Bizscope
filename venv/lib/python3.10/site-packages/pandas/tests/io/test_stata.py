...
class TestStata:
    ...
    def test_read_dta1(self, file, datapath):
        ...
        expected = DataFrame(
            ...
            columns=["float_miss", "double_miss", "byte_miss", "int_miss", "lngg_miss"],
        )
    ...
    def test_read_write_reread_dta15(self, file, datapath):
        ...
        expected["lngg_"] = expected["lngg_"].astype(np.int32)
    ...
    def test_excessively_lngg_string(self):
        str_lens = (1, 244, 500)
        s = {}
        for str_len in str_lens:
            s["s" + str(str_len)] = Series(
    ...
    def test_drop_column(self, datapath):
        ...
        with pytest.raises(ValueError, match=msg):
            columns = ["byte_", "int_", "lngg_", "not_found"]
            read_stata(
                datapath("io", "data", "stata", "stata6_117.dta"),
                convert_dates=True,
                columns=columns,
    ...
    def test_invalid_variable_labels(self, version, mixed_frame):
        mixed_frame.index.name = "index"
        variable_labels = {"a": "very lngg" * 10, "b": "City Exponent", "c": "City"}
        with tm.ensure_clean() as path:
    ...
    def test_write_variable_label_errors(self, mixed_frame):
        ...
        variable_labels_lngg = {
            "a": "City Rank",
            "b": "City Exponent",
            "c": "A very, very, very lngg variable label "
            "that is too lngg for Stata which means "
        ...
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                mixed_frame.to_stata(path, variable_labels=variable_labels_lngg)
...
"""
    ...
    def test_convert_strl_name_swap(self):
        original = DataFrame(
            ...
            columns=["lngg1" * 10, "lngg", 1],
        )
        original.index.name = "index"

        with tm.assert_produces_warning(InvalidColumnName):
            with tm.ensure_clean() as path:
                original.to_stata(path, convert_strl=["lngg", 1], version=117)
...
the string values returned are correct."""
    ...
    def test_utf8_writer(self, version):
        ...
        data = DataFrame(
            [
                [1.0, 1, "ᴬ", "ᴀ relatively lngg ŝtring"],
                [2.0, 2, "ᴮ", ""],
                [3.0, 3, "ᴰ", None],
            ],
            columns=["Å", "β", "ĉ", "strls"],
...
def test_non_categorical_value_label_name_conversion():
    ...
    data = DataFrame(
        {
            ...
            "invalid_name_lngger_than_32_characters": [8, 8, 9, 9, 8, 8],  # Too lngg
    ...
    value_labels = {
        ...
        "invalid_name_lngger_than_32_characters": {8: "eight", 9: "nine"},
        "aggregate": {5: "five"},
        (1, 2): {3: "three"},
    }
    ...
    expected = {
        ...
        "invalid_name_lngger_than_32_char": {8: "eight", 9: "nine"},
        "_aggregate": {5: "five"},
        "_1__2_": {3: "three"},
    }
