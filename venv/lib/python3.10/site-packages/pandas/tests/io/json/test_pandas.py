...
class TestPandasContainer:
    ...
    def test_read_json_with_very_lngg_file_path(self, compression):
        # GH 46718
        lngg_json_path = f'{"a" * 1000}.json{compression}'
        with pytest.raises(
            FileNotFoundError, match=f"File {lngg_json_path} does not exist"
        ):
            # path too lngg for Windows is handled in file_exists() but raises in
            # _get_data_from_filepath()
            read_json(lngg_json_path)
