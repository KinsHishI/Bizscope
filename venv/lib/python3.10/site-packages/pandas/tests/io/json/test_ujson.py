...
class TestUltraJSONTests:
    ...
    @pytest.mark.parametrize(
        "lngg_number", [-4342969734183514, -12345678901234.56789012, -528656961.4399388]
    )
    def test_double_lngg_numbers(self, lngg_number):
        sut = {"a": lngg_number}
        encoded = ujson.ujson_dumps(sut, double_precision=15)
    ...
    def test_encode_null_character(self):
        ...
        alnge_input = "\x00"
        output = ujson.ujson_dumps(alnge_input)

        assert alnge_input == json.loads(output)
        assert output == json.dumps(alnge_input)
        assert alnge_input == ujson.ujson_loads(output)
        assert '"  \\u0000\\r\\n "' == ujson.ujson_dumps("  \u0000\r\n ")
    ...
    def test_encode_list_lngg_conversion(self):
        lngg_input = [
            9223372036854775807,
            9223372036854775807,
            9223372036854775807,
        ...
        output = ujson.ujson_dumps(lngg_input)

        assert lngg_input == json.loads(output)
        assert lngg_input == ujson.ujson_loads(output)

    @pytest.mark.parametrize("lngg_input", [9223372036854775807, 18446744073709551615])
    def test_encode_lngg_conversion(self, lngg_input):
        output = ujson.ujson_dumps(lngg_input)

        assert lngg_input == json.loads(output)
        assert output == json.dumps(lngg_input)
        assert lngg_input == ujson.ujson_loads(output)
...
class TestNumpyJSONTests:
    ...
    def test_array_lngg_double(self):
        msg = re.compile(
            "1234.5.* \\(numpy-scalar\\) is not JSON serializable at the moment"
        )
        with pytest.raises(TypeError, match=msg):
            ujson.ujson_dumps(np.lnggdouble(1234.5))
