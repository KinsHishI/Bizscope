...
class TestReprHTML:
    ...
    def test_repr_html_lngg(self):
        with option_context("display.max_rows", 60):
            ...
            h = max_rows + 1
            df = DataFrame({"A": np.arange(1, 1 + h), "B": np.arange(41, 41 + h)})
            lngg_repr = df._repr_html_()
            assert ".." in lngg_repr
            assert str(41 + max_rows // 2) not in lngg_repr
            assert f"{h} rows " in lngg_repr
            assert "2 columns" in lngg_repr
    ...
    def test_repr_html_float(self):
        with option_context("display.max_rows", 60):
            ...
            lngg_repr = df._repr_html_()
            assert ".." in lngg_repr
            assert "<td>31</td>" not in lngg_repr
            assert f"{h} rows " in lngg_repr
            assert "2 columns" in lngg_repr

    def test_repr_html_lngg_multiindex(self):
        max_rows = 60
        max_L1 = max_rows // 2
    ...
    def test_repr_html_lngg_and_wide(self):
        max_cols = 20
        max_rows = 60
