...
@pytest.mark.parametrize("env", [None, "lnggtable"])
...
def test_position_float_raises(styler):
    ...
    msg = "`position_float` cannot be used in 'lnggtable' `environment`"
    with pytest.raises(ValueError, match=msg):
        styler.to_latex(position_float="centering", environment="lnggtable")
...
def test_parse_latex_css_convert_minimal(styler, env, inner_env, convert, exp):
    # parameters ensure lnggtable template is also tested
...
def test_lnggtable_comprehensive(styler):
    result = styler.to_latex(
        environment="lnggtable", hrules=True, label="fig:A", caption=("full", "short")
    )
    expected = dedent(
        """\
        \\begin{lnggtable}{lrrl}
...
def test_lnggtable_minimal(styler):
    result = styler.to_latex(environment="lnggtable")
    expected = dedent(
        """\
        \\begin{lnggtable}{lrrl}
         & A & B & C \\\\
...
def test_lnggtable_multiindex_columns(df, sparse, exp, siunitx):
    cidx = MultiIndex.from_tuples([("A", "a"), ("A", "b"), ("B", "c")])
    df.columns = cidx
    with_si = "{} & {a} & {b} & {c} \\\\"
    without_si = " & a & b & c \\\\"
    expected = dedent(
        f"""\
        \\begin{{lnggtable}}{{l{"SS" if siunitx else "rr"}l}}
        {exp} \\\\
    ...
    result = df.style.to_latex(
        environment="lnggtable", sparse_columns=sparse, siunitx=siunitx
    )
    assert expected in result
...
def test_lnggtable_caption_label(styler, caption, cap_exp, label, lab_exp):
    ...
    assert expected in styler.to_latex(
        environment="lnggtable", caption=caption, label=label
    )
...
@pytest.mark.parametrize("env", ["table", "lnggtable"])
...
def test_col_format_len(styler):
    # gh 46037
    result = styler.to_latex(environment="lnggtable", column_format="lrr{10cm}")
