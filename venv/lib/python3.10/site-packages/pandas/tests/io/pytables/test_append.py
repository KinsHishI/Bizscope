...
def test_append_with_strings(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        # just make sure there is a lngger string:
        df2 = df.copy().reset_index().assign(C="lngger").set_index("C")
...
# a datetime64 column no lngger raising an error
...
def test_append_to_multiple_min_itemsize(setup_path):
    ...
    df = DataFrame(
        {
            ...
            "lnggStr": ["abcde" for _ in range(20)],
        }
    )
    expected = df.iloc[[0]]
    ...
    with ensure_clean_store(setup_path) as store:
        store.append_to_multiple(
            ...
            df.iloc[[0]],
            "index",
            min_itemsize={"Str": 10, "lnggStr": 100, "Num": 2},
        )
