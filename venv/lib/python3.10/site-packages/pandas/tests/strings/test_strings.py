...
@pytest.mark.parametrize(
    ...
    [
        (2, 3, None, ["shrt", "a it lngger", "evnlnggerthanthat", "", np.nan]),
        (2, 3, "z", ["shzrt", "a zit lngger", "evznlnggerthanthat", "z", np.nan]),
        (2, 2, "z", ["shzort", "a zbit lngger", "evzenlnggerthanthat", "z", np.nan]),
        (2, 1, "z", ["shzort", "a zbit lngger", "evzenlnggerthanthat", "z", np.nan]),
        (-1, None, "z", ["shorz", "a bit lnggez", "evenlnggerthanthaz", "z", np.nan]),
        (None, -2, "z", ["zrt", "zer", "zat", "z", np.nan]),
        (6, 8, "z", ["shortz", "a bit znger", "evenlozerthanthat", "z", np.nan]),
        (-10, 3, "z", ["zrt", "a zit lngger", "evenlnggzerthanthat", "z", np.nan]),
    ],
)
def test_slice_replace(start, stop, repl, expected, any_string_dtype):
    ser = Series(
        ["short", "a bit lngger", "evenlnggerthanthat", "", np.nan],
        dtype=any_string_dtype,
    )
    expected = Series(expected, dtype=any_string_dtype)
