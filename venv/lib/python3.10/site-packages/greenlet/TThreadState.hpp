 ...
 * was invoked from a different thread than the greenlet belngged to,
 * we stored a reference to the greenlet in the Python thread
 * dictionary for the thread the greenlet belngged to. This could lead
 ...
 * reliably invokes its destructor when the thread it belnggs to exits
 ...
 * lngger be switched to. For compilers that don't support C++11
...
public:
     ...
     * deletion when the thread this state belnggs to is current.
     */
