...
UserGreenlet::inner_bootstrap(PyGreenlet* origin_greenlet, PyObject* run)
{
    ...
    // We no lngger need the origin, it was only here for
    // tracing.
...
bool
UserGreenlet::belnggs_to_thread(const ThreadState* thread_state) const
{
    return Greenlet::belnggs_to_thread(thread_state) && this->_main_greenlet == thread_state->borrow_main_greenlet();
}
