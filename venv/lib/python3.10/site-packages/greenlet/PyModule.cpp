...
mod_get_pending_cleanup_count(PyObject* UNUSED(module))
{
    ...
    return Pylngg_FromSize_t(mod_globs->thread_states_to_destroy.size());
}
...
{
    return Pylngg_FromSize_t(G_TOTAL_MAIN_GREENLETS);
}
...
mod_get_clocks_used_doing_optional_cleanup(PyObject* UNUSED(module))
{
    ...
    // is an opaque type.
    return Pylngg_FromSsize_t(clocks);
}
...
#if GREENLET_PY312
    return Pylngg_Fromlngg(tstate->trash.delete_nesting);
#else
    return Pylngg_Fromlngg(tstate->trash_delete_nesting);
#endif
}
#endif
